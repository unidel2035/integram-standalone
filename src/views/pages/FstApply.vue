    ...
    <div v-if="!submitted" class="apply-form-wrap">
      ...
      <div v-if="currentStep === 0" class="step-form">
        ...
        <div class="form-grid">
          ...
          <div class="form-field full">
            <label>Краткое описание технологии *</label>
            <textarea v-model="form.description" rows="3" placeholder="Опишите вашу технологию в 2-3 предложениях..."></textarea>
          </div>
        </div>
      </div>

      <!-- Шаг 2: Технология и рынок -->
      <div v-if="currentStep === 1" class="step-form">
        <h2>Технология и рынок</h2>
        <div class="form-grid">
          <div class="form-field">
          ...
          <div class="form-field">
            <label>Патенты / ИС</label>
            <input v-model="form.patents" placeholder="Количество и статус патентов" />
          </div>
          <div class="form-field">
            <label>Объём рынка TAM, млрд ₽</label>
            <input v-model.number="form.tam" type="number" step="1" />
          </div>
          <div class="form-field">
            <label>Объём рынка SAM, млрд ₽</label>
            <input v-model.number="form.sam" type="number" step="0.5" />
          </div>
          <div class="form-field">
            <label>Ключевые конкуренты</label>
            <input v-model="form.competitors" placeholder="Главные игроки на рынке" />
          </div>
          <div class="form-field">
            <label>УТП (в чём уникальность?)</label>
            <textarea v-model="form.usp" rows="2" placeholder="Ваше конкурентное преимущество..."></textarea>
          </div>
          <div class="form-field full">
            <label>Суверенность технологии (% отечественных компонентов)</label>
            <div class="sovereignty-slider-wrap">
      ...
      <div v-if="currentStep === 2" class="step-form">
        ...
        <div class="form-grid">
          ...
          <div class="form-field">
            <label>Runway до исчерпания средств, мес.</label>
          ...
          <div class="form-field full">
            <label>Ключевые достижения команды</label>
            <textarea v-model="form.achievements" rows="3" placeholder="Предыдущие проекты, экспертиза, награды..."></textarea>
          </div>
        </div>
      </div>
...
const form = ref({
  companyName: '', inn: '', sector: '', stage: '', foundedYear: null, city: '',
  description: '', trl: 5, patents: '', tam: null, sam: null, competitors: '', usp: '',
  sovereignty: 70, amount: 100, equityOffered: 15, preMoney: null, arr: null,
  runway: null, teamSize: null, ceoName: '', achievements: '',
...
const scoringRows = computed(() => [
  ...
  { label: 'Рынок (SAM)', score: form.value.sam ? Math.min(100, form.value.sam * 5) : 20, color: '#ab47bc' }
])
...
<style scoped>
.apply-root { padding: 24px; display: flex; flex-direction: column; gap: 24px; min-height: 100vh; background: var(--p-surface-ground); }

.apply-hero { background: var(--p-content-background); border: 1px solid var(--p-surface-border); border-radius: 12px; padding: 32px; text-align: center; }
...
.ac-item { display: flex; align-items: center; gap: 10px; background: var(--p-surface-ground); border: 1px solid var(--p-surface-border); border-radius: 8px; padding: 10px 14px; }
...
.apply-form-wrap { background: var(--p-content-background); border: 1px solid var(--p-surface-border); border-radius: 12px; padding: 28px; display: flex; flex-direction: column; gap: 24px; }

.apply-steps { display: flex; gap: 0; overflow-x: auto; }
.apply-step { display: flex; align-items: center; gap: 8px; flex: 1; position: relative; }
.apply-step::after { content: '→'; position: absolute; right: -8px; color: var(--p-text-muted-color); font-size: 0.8rem; }
.apply-step:last-child::after { display: none; }
.step-circle { width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.82rem; font-weight: 700; background: var(--p-surface-ground); border: 2px solid var(--p-surface-border); color: var(--p-text-muted-color); flex-shrink: 0; }
...
.step-label { font-size: 0.78rem; color: var(--p-text-muted-color); }
...
.trl-slider-wrap, .sovereignty-slider-wrap { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.trl-slider { flex: 1; accent-color: var(--p-primary-color); }
...
.file-drop { display: flex; align-items: center; gap: 10px; border: 2px dashed var(--p-surface-border); border-radius: 8px; padding: 12px; }
.file-btn { padding: 6px 12px; border-radius: 6px; border: 1px solid var(--p-surface-border); background: var(--p-surface-card); color: var(--p-text-color); cursor: pointer; font-size: 0.82rem; }
.file-name { font-size: 0.78rem; color: var(--p-text-muted-color); }

.scoring-preview { margin-top: 8px; background: var(--p-surface-ground); border: 1px solid var(--p-surface-border); border-radius: 10px; padding: 16px; }
...
.step-nav { display: flex; justify-content: space-between; padding-top: 8px; border-top: 1px solid var(--p-surface-border); }
...
.apply-btn.secondary { background: var(--p-surface-card); color: var(--p-text-color); border: 1px solid var(--p-surface-border); }
.apply-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.apply-btn.submit { background: #66bb6a; }

.apply-success { background: var(--p-content-background); border: 1px solid var(--p-surface-border); border-radius: 12px; padding: 48px; text-align: center; }
