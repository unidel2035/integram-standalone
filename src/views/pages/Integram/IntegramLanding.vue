<template>
  <div class="integram-landing-container">
    ...
    <!-- Create New Database Section (only on 'my' database) -->
    <Card v-if="database === 'my'" class="mb-4">
      <template #title>
        <div class="flex align-items-center gap-2">
          <i class="pi pi-plus-circle"></i>
          <span>{{ t('createDb') }}</span>
        </div>
      </template>
      <template #subtitle>{{ t('createDbDesc') }}</template>
      <template #content>
        <div class="flex gap-3 align-items-end flex-wrap">
          <div class="flex flex-column gap-1">
            <label class="text-sm font-semibold">{{ t('dbName') }}</label>
            <InputText
              v-model="newDbName"
              :placeholder="t('dbNamePlaceholder')"
              :invalid="!!newDbNameError"
              class="w-15rem"
              @input="validateDbName"
            />
            <small v-if="newDbNameError" class="text-red-500">{{ newDbNameError }}</small>
          </div>
          <div class="flex flex-column gap-1">
            <label class="text-sm font-semibold">{{ t('dbTemplate') }}</label>
            <Dropdown
              v-model="newDbTemplate"
          ...
          <div class="flex flex-column gap-1" style="flex: 1; min-width: 200px;">
            <label class="text-sm font-semibold">{{ t('dbDescription') }}</label>
            <InputText
              v-model="newDbDescription"
              :placeholder="t('dbDescriptionPlaceholder')"
            />
          </div>
          <Button
            :label="t('createDbBtn')"
            icon="pi pi-plus"
            :loading="creatingDb"
            :disabled="!newDbName || !!newDbNameError"
...
const newDbName = ref('')
const newDbTemplate = ref('RU')
const newDbDescription = ref('')
const newDbNameError = ref('')
const creatingDb = ref(false)

const templateOptions = [
  { label: 'Русский', value: 'RU' },
  { label: 'Пустая финмодель', value: 'FM' },
  { label: 'Финмодель 3 примера', value: 'MO' },
  { label: 'English', value: 'EN' },
  { label: 'Full with HTMLs', value: 'FU' }
]

function validateDbName() {
  const name = newDbName.value
  if (!name) {
    newDbNameError.value = ''
    return
  }
  if (!/^[a-zA-Z]/.test(name)) {
    newDbNameError.value = 'Должно начинаться с буквы'
  } else if (!/^[a-zA-Z][a-zA-Z0-9]*$/.test(name)) {
    newDbNameError.value = 'Только латинские буквы и цифры'
  } else if (name.length < 3) {
    newDbNameError.value = 'Минимум 3 символа'
  } else if (name.length > 15) {
    newDbNameError.value = 'Максимум 15 символов'
  } else {
    newDbNameError.value = ''
  }
}
...
async function handleCreateDatabase() {
  if (!newDbName.value || newDbNameError.value) return

  creatingDb.value = true
  try {
    const result = await integramApiClient.createDatabase(
      newDbName.value,
      newDbTemplate.value,
    ...
    toast.add({
      severity: 'success',
      summary: 'База создана',
      detail: `База "${newDbName.value}" успешно создана`,
      life: 3000
    })

    // Переключаемся на новую базу
    const dbName = newDbName.value.toLowerCase()
    newDbName.value = ''
    newDbDescription.value = ''
    ...
    toast.add({
      ...
      detail: error.message || 'Не удалось создать базу данных',
      life: 5000
    })
  } finally {
...
function t(key) {
  const translations = {
    ru: {
      ...
      createDb: 'Создать новую базу',
      createDbDesc: 'Создайте новую базу данных Integram',
      dbName: 'Имя базы',
      dbNamePlaceholder: 'memory',
      dbTemplate: 'Шаблон',
      dbDescription: 'Описание',
      dbDescriptionPlaceholder: 'Описание базы данных...',
      createDbBtn: 'Создать'
    },
    en: {
      welcome: 'Welcome to',
      ...
      createDbDesc: 'Create a new Integram database',
      dbName: 'Database Name',
      dbNamePlaceholder: 'memory',
      dbTemplate: 'Template',
      dbDescription: 'Description',
      dbDescriptionPlaceholder: 'Database description...',
      createDbBtn: 'Create'
    }
  }
