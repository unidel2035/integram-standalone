<template>
  <FstPageLayout title="Сценарии выхода" subtitle="M&A, IPO, Buyback — расчёт доходности и оптимальной стратегии">
    ...
    <div class="exit-company-bar">
      <SelectButton v-model="selectedCo" :options="companies" optionLabel="name" optionValue="id" :allowEmpty="false" />
    </div>

    <!-- Параметры выхода -->
    <div class="exit-params-card">
      <div class="exit-section-title">Входные параметры — {{ currentCompany.name }}</div>
      <div class="params-grid">
        <div class="param-group">
          <label>Текущий NAV, млн ₽</label>
          <InputNumber v-model="params.nav" :step="10" :min="0" :showButtons="false" fluid @input="calc" />
        </div>
        <div class="param-group">
          <label>Вложено ФСТ, млн ₽</label>
          <InputNumber v-model="params.invested" :step="10" :min="0" :showButtons="false" fluid @input="calc" />
        </div>
        <div class="param-group">
          <label>Доля ФСТ FD, %</label>
          <InputNumber v-model="params.fstShare" :step="0.5" :min="0" :max="100" suffix="%" :showButtons="false" fluid @input="calc" />
        </div>
        <div class="param-group">
        ...
        <div class="param-group">
          ...
          <InputNumber v-model="params.revenue" :step="10" :min="0" :showButtons="false" fluid @input="calc" />
        </div>
        <div class="param-group">
          <label>ARR Growth, %</label>
          <InputNumber v-model="params.growth" :step="5" suffix="%" :showButtons="false" fluid @input="calc" />
        </div>
      </div>
    </div>
    ...
    <div class="exit-scenarios">
      ...
      <div class="exit-scenario-card" :class="{ best: bestScenario === 'ma' }">
        ...
        <div class="exit-section-title">Стратегическое поглощение</div>
        <div class="scenario-params">
          <div class="sp-group">
            <label>EV/Revenue (стратег.)</label>
            <InputNumber v-model="maParams.evRevMult" :step="0.5" :min="1" :showButtons="false" fluid @input="calc" />
          </div>
          <div class="sp-group">
            <label>Горизонт, лет</label>
            <InputNumber v-model="maParams.years" :step="1" :min="1" :max="7" :showButtons="false" fluid @input="calc" />
          </div>
          <div class="sp-group">
            <label>Вероятность, %</label>
            <InputNumber v-model="maParams.prob" :step="5" :min="5" :max="95" suffix="%" :showButtons="false" fluid @input="calc" />
          </div>
        </div>
      ...
      <div class="exit-scenario-card" :class="{ best: bestScenario === 'ipo' }">
        ...
        <div class="exit-section-title">Первичное размещение</div>
        <div class="scenario-params">
          <div class="sp-group">
            <label>P/S при IPO</label>
            <InputNumber v-model="ipoParams.psMult" :step="0.5" :min="1" :showButtons="false" fluid @input="calc" />
          </div>
          <div class="sp-group">
            <label>Lock-up, мес.</label>
            <InputNumber v-model="ipoParams.lockupMonths" :step="3" :min="3" :max="24" :showButtons="false" fluid @input="calc" />
          </div>
          <div class="sp-group">
            <label>Вероятность, %</label>
            <InputNumber v-model="ipoParams.prob" :step="5" :min="5" :max="60" suffix="%" :showButtons="false" fluid @input="calc" />
          </div>
        </div>
        <div class="scenario-results">
          <div class="sr-row">
      ...
      <div class="exit-scenario-card" :class="{ best: bestScenario === 'bb' }">
        ...
        <div class="exit-section-title">Выкуп основателями / Вторичка</div>
        <div class="scenario-params">
          <div class="sp-group">
            <label>Дисконт к NAV, %</label>
            <InputNumber v-model="bbParams.navDiscount" :step="5" :min="0" :max="50" suffix="%" :showButtons="false" fluid @input="calc" />
          </div>
          <div class="sp-group">
            <label>Горизонт, лет</label>
            <InputNumber v-model="bbParams.years" :step="1" :min="1" :max="5" :showButtons="false" fluid @input="calc" />
          </div>
          <div class="sp-group">
            <label>Вероятность, %</label>
            <InputNumber v-model="bbParams.prob" :step="5" :min="10" :max="90" suffix="%" :showButtons="false" fluid @input="calc" />
          </div>
        </div>
        <div class="scenario-results">
          <div class="sr-row">
    ...
    <div class="exit-ev-card">
      <div class="exit-section-title">Взвешенный ожидаемый результат (EV)</div>
      <div class="ev-table">
...
<style scoped>
.exit-content { display: flex; flex-direction: column; gap: 16px; padding-top: 16px; }
.exit-section-title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: var(--p-text-muted-color); margin-bottom: 12px; }

.exit-company-bar { display: flex; }

.exit-params-card { background: var(--p-surface-card); border: 1px solid var(--p-content-border-color); border-radius: 12px; padding: 20px; }
.params-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 12px; }
.param-group { display: flex; flex-direction: column; gap: 4px; min-width: 0; }
...
.exit-scenario-card { background: var(--p-surface-card); border: 1px solid var(--p-content-border-color); border-radius: 12px; padding: 16px; display: flex; flex-direction: column; gap: 12px; }
...
.sp-group { display: flex; flex-direction: column; gap: 3px; min-width: 0; }
...
.exit-ev-card { background: var(--p-surface-card); border: 1px solid var(--p-content-border-color); border-radius: 12px; padding: 20px; }
