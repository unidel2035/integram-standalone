...
/**
 * Detect if an error is caused by a stale/missing chunk after deploy
 * These errors are not recoverable by retrying the same URL
 */
 ...
 * Aborts immediately for stale chunk errors (dead URLs won't recover via retry)
 *
...
export async function retryDynamicImport(importFn, maxRetries = 3, baseDelay = 1000) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Stale chunk = file no longer exists on server. Retrying is pointless.
      if (isStaleChunkError(error)) {
        logger.warn('🔗 Stale chunk detected — skipping remaining retries')
        break
      }
  ...
  // All retries failed (or stale chunk detected)
  ...
  enhancedError.isStaleChunk = isStaleChunkError(lastError)
