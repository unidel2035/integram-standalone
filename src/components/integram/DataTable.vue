<template>
  <div class="coda-style-datatable" :class="{ 'text-wrap-enabled': textWrapEnabled, 'text-wrap-disabled': !textWrapEnabled }">
      ...
      <div class="row-edit-form">
        <div v-for="header in editingRow?.headers" :key="header.headerId" class="form-field" :class="{'has-expanding-content': isExpandingField(header.type)}" @dblclick.stop="handleCellDoubleClick(header, row.cells[header.id])">
          ...
          <template v-else-if="header.dirTableId">
            <MultiSelect v-if="header.columnType === 'multi'" v-model="header.dirValues" :options="getDirectoryOptions(header.dirTableId, header.filterCondition, editingRow)" option-label="value" option-value="id" :multiple="true" :loading="isDirectoryLoading(header.dirTableId)" :disabled="isDirectoryLoading(header.dirTableId)" :placeholder="isDirectoryLoading(header.dirTableId) ? 'Загрузка...' : 'Выберите...'" class="w-full" @change="updateMultiDirValue(header)" />
            <Dropdown v-else v-model="header.dirRowId" :options="getDirectoryOptions(header.dirTableId, header.filterCondition, editingRow)" option-label="value" option-value="id" :filter="true" :show-clear="true" :loading="isDirectoryLoading(header.dirTableId)" :disabled="isDirectoryLoading(header.dirTableId)" :placeholder="isDirectoryLoading(header.dirTableId) ? 'Загрузка...' : 'Выберите...'" class="w-full" @change="updateDirValue(header)" />
          </template>
    ...
    <!-- AI Agent Config Dialog (for ai-agent buttons) -->
    <AIAgentConfigDialog
      v-model:visible="isAIAgentConfigDialogVisible"
      :config="aiAgentCurrentConfig"
      :headers="localHeaders"
      :row-data="aiAgentConfigRow?.cells || {}"
      @save="handleAIAgentConfigSave"
      @cancel="closeAIAgentConfigDialog"
    />

    <!-- Select Column Settings Dialog (for dir/multi columns) -->
    <SelectColumnSettingsDialog
      v-model:visible="isSelectColumnSettingsDialogVisible"
      :config="selectColumnCurrentConfig"
      :header="selectColumnConfigHeader"
      :is-multi="selectColumnConfigHeader?.isMulti ?? false"
      :directory-fields="selectColumnDirectoryFields"
      :directory-preview="selectColumnDirectoryPreview"
      :current-table-fields="localHeaders"
      @save="handleSelectColumnSettingsSave"
      @cancel="closeSelectColumnSettingsDialog"
    />

    <!-- Number Format Settings Dialog (Issue #6867) -->
    <NumberFormatSettingsDialog
      v-model:visible="isNumberFormatDialogVisible"
      :config="numberFormatCurrentConfig"
      @save="handleNumberFormatSave"
      @cancel="closeNumberFormatDialog"
    />

    <!-- Vote Config Dialog (Issue #6877) -->
    <Dialog
      v-model:visible="isVoteConfigDialogVisible"
      header="Настройка иконки голосования"
      :style="{ width: '400px' }"
      :modal="true"
    >
      <div class="vote-config-form">
        <div class="form-field">
          <label>Иконка</label>
          <div class="icon-selector">
            <InputText
              v-model="voteIconInput"
              placeholder="👍"
              class="icon-input"
            />
            <div class="icon-preview">{{ voteIconInput || '👍' }}</div>
          </div>
          <small class="help-text">Введите любой эмодзи: 👍 ⭐ ✅ ▲ 👏 ❤️</small>
        </div>
      </div>
      <template #footer>
        <Button label="Отмена" icon="pi pi-times" @click="closeVoteConfigDialog" text />
        <Button label="Сохранить" icon="pi pi-check" @click="saveVoteConfig" />
      </template>
    </Dialog>
    ...
    <!-- Reference Table Modal (Issue #6859) -->
    <Dialog
      v-model:visible="isReferenceTableModalVisible"
      :header="'Выбрать из справочника: ' + referenceTableName"
      ...
      :maximizable="true"
      appendTo="body"
      class="reference-table-modal"
    >
      <IntegramDataTableWrapper
        v-if="referenceTableId && isReferenceTableModalVisible"
        ref="referenceTableWrapperRef"
        :key="'ref-table-' + referenceTableId"
        :database="database"
        :typeId="referenceTableId"
        :serverUrl="serverUrl"
        :token="token"
        :xsrfToken="xsrfToken"
        :embedded="true"
      ...
      <template #footer>
        <Button
          label="Закрыть"
          icon="pi pi-times"
          @click="isReferenceTableModalVisible = false"
          text
        />
        <Button
          label="Выбрать"
          ...
          :disabled="!hasReferenceSelection"
        />
      </template>
    </Dialog>

    <!-- Row Detail / Edit Dialog (reuses IntegramEnhancedObjectEditor) -->
    <Dialog
      v-model:visible="rowDetailVisible"
      ...
      :maximizable="true"
      appendTo="body"
      class="row-detail-editor-dialog"
    >
      <IntegramEnhancedObjectEditor
        v-if="rowDetailObjectId"
        :database="database"
        :objectId="rowDetailObjectId"
      />
    </Dialog>

    <ContextMenu ref="headerContextMenu" :model="headerMenuItems" />
    <TieredMenu ref="headerTieredMenu" :model="headerTieredMenuItems" :popup="true" @hide="onHeaderMenuHide" />
    ...
    <div class="table-container" :id="'table-container-' + $attrs['data-tab-id']" @scroll="handleScroll" @mousedown="startSelection" @mousemove="handleDragSelection" @mouseup="endSelection">
...
<th 
  ...
  :class="{
    ...
    'col-menu-active': currentMenuHeader?.id === header.id
  }"
  :title="header.value"
>
  <div class="header-content" @mouseenter="onHeaderMouseEnter(header.id)" @mouseleave="onHeaderMouseLeave">
    ...
    <template v-else>
      <div class="header-text-wrapper">
        ...
        <!-- Compact indicators for active states -->
        <span v-if="isColumnSorted(header.id)" class="sort-dot" :title="'Сортировка ' + (getSortInfo(header.id).direction === 'asc' ? '↑' : '↓') + ' (' + getSortInfo(header.id).order + ')'">
          {{ getSortInfo(header.id).direction === 'asc' ? '↑' : '↓' }}<sup>{{ getSortInfo(header.id).order }}</sup>
        </span>
        <span v-if="hasActiveColumnFilter(header.id)" class="filter-dot" title="Фильтр активен">●</span>
      </div>
      <!-- Column menu button — показывается при наведении или при открытом меню -->
      <button
        v-show="hoveredHeaderId === header.id || currentMenuHeader?.id === header.id"
        class="header-menu-btn"
        @click.stop="openHeaderMenu($event, header)"
        title="Меню колонки"
      >
  ...
  <div
    ...
    @click.stop
    v-tooltip.left="{ value: 'Двойной клик — автоподбор/сброс ширины', showDelay: 1000 }"
  ></div>
</th>
            <!-- Add column button - Notion/Coda style -->
            <!-- Hidden during loading (global preloader or background loading) to avoid flicker -->
            <!-- Also show for empty tables (subordinate tables in modal) -->
            <th class="add-column-header" v-if="!props?.disableEditing && !props?.disableTypeEditing && !props?.isLoading && !props?.isLoadingMore">
              <button
        ...
        <tbody>
          ...
          <template v-for="(item, itemIndex) in visibleRows" :key="item.type === 'group' ? `group-${item.key}` : item.data.id">
            ...
            <tr
              ...
              class="data-row"
              :class="{ 'row-is-selected': selectedRowIds.has(item.data.id) }"
              :style="props.textWrapEnabled && rowHeights[item.data.id] ? { height: `${rowHeights[item.data.id]}px` } : { height: '36px' }"
              @focusin="handleRowFocus($event, item.data, props.autoLoadDirs)"
              @mouseenter="_rowMouseEnter(item.data.id); handleRowHover(item.data, true)"
              @mouseleave="_rowMouseLeave()"
              @contextmenu.prevent="onRowContextMenu($event, item.data)"
            >
              <td class="row-counter-cell" :style="{ ...rowCounterStyle, ...(props.textWrapEnabled && rowHeights[item.data.id] ? { height: `${rowHeights[item.data.id]}px` } : { height: '36px', maxHeight: '36px' }) }" :class="{'row-selected': selectedRowIds.has(item.data.id), 'hover-active': hoveredRowId === item.data.id}">
                <!-- Row Resizer Handle -->
                <div
                  class="row-resizer"
                ...
                ></div>
                <!-- Row counter cell content -->
                <div class="cell-content row-counter-unified" :class="{ 'selection-mode-active': selectionModeEnabled, 'row-selected-coda': selectedRowIds.has(item.data.id) }">
                  <!-- Expand icon: absolute left edge, visible on row hover -->
                  <button
                    class="row-expand-btn"
                    @click.stop="openRowDetail(item.data)"
                    v-tooltip.right="{ value: 'Развернуть строку', showDelay: 300 }"
                    title="Развернуть"
                  >
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                      <rect x="1" y="1" width="14" height="14" rx="2" stroke="currentColor" stroke-width="1.3"/>
                      <path d="M9.5 2.5H13.5V6.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                      <path d="M13.5 2.5L9 7" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
                    </svg>
                  </button>
                  <!-- Grip (6 dots): replaces row number on hover -->
                  <button
                    v-if="!props?.disableEditing"
                    class="row-grip-btn"
                    @click.stop="openRowActionsMenu($event, item.data)"
                    v-tooltip.right="{ value: 'Действия со строкой', showDelay: 300 }"
                    title="Действия"
                  >
                  ...
                  <Checkbox
                    class="row-select-checkbox-coda"
                    ...
                    v-tooltip.right="{ value: 'Выделить строку', showDelay: 700 }"
                  />
                  <!-- Row number: default state, right-aligned -->
                  <span class="row-number-coda" @click.stop="startRowEdit(item.data)">
                    {{ findRowIndex(item.data.id) + 1 }}
                  </span>
                </div>
                <!-- Add row: outside the inner div, positioned relative to td so translateY(50%) centers exactly on border -->
                <button
                  v-if="!props?.disableEditing"
                  v-show="hoveredRowId === item.data.id"
                  class="row-add-inline-btn"
                  @click.stop="emitAddRowAfter(item.data)"
                  @mouseenter.stop="_rowMouseEnter(item.data.id)"
                  @mouseleave="_rowMouseLeave()"
                  title="Добавить строку ниже"
                  v-tooltip.bottom="{ value: 'Новая строка', showDelay: 500 }"
                >
                  <i class="pi pi-plus"></i>
                </button>
              </td>
              <template v-for="header in localHeaders" :key="header.id">
              <td
                v-if="shouldRenderCell(header.id, item.data.id)"
                :data-cell-id="item.data.cells[header.id]?.id"
                :data-row-id="item.data.id"
                :data-header-id="header.id"
                :data-label="header.value"
                :rowspan="getCellRowspan(header.id, item.data.id)"
                :style="{
                  width: `${header.width}px`,
                  ...(isPinnedColumn(header.id) ? { left: `${pinnedColumnsOffsets[header.id]}px` } : {}),
                  ...getCellStyle(header.id, item.data.cells[header.id]?.value, item.data.id),
                  ...(props.textWrapEnabled && rowHeights[item.data.id] ? { height: `${rowHeights[item.data.id]}px` } : { height: '36px', maxHeight: '36px' }),
                  ...(getCellRowspan(header.id, item.data.id) > 1 ? { verticalAlign: 'middle' } : {})
                }"
                :class="{
                  ...
                  'merged-cell': getCellRowspan(header.id, item.data.id) > 1,
                  'col-menu-active': currentMenuHeader?.id === header.id,
                  'fill-target': isFillTarget(header.id, item.data.id),
                  'is-select-cell': header.columnType === 'dir' || header.columnType === 'multi'
                }"
                @mousedown.stop="handleCellMousedown($event, header.id, item.data.id)"
                @dblclick.stop="startCellEdit(header.id, item.data.id)"
              >
                <!-- Fill handle: dot at corner, own mousedown handler -->
                <div
                  v-if="isCellSelected(header.id, item.data.id)"
                  class="fill-handle"
                  @mousedown.stop.prevent="startFillDrag($event, header.id, item.data.id)"
                ></div>
                <div class="cell-content">
                  <template v-if="!isInlineEditing(header.id, item.data.id)">
                    ...
                    <div v-else-if="header.columnType === 'dir'" class="cell-dir" @mousedown="handleDirCellMousedown(header.id, item.data.id)" @click.stop="handleDirCellClick(header.id, item.data.id)" @dblclick.stop="startCellEdit(header.id, item.data.id)">
                      <span v-if="item.data.cells[header.id].dirRowId && item.data.cells[header.id].value" class="dir-single-tag" @mouseenter="showDirInfo($event, header, item.data.cells[header.id].dirRowId)" @mouseleave="hideDirInfo">
                        {{ item.data.cells[header.id].value }}
                        <i v-if="!props.disableEditing" class="pi pi-times dir-single-clear" @mousedown.stop @click.stop="clearDirValue(header, item.data.id)"></i>
                      </span>
                      <span v-else class="cell-empty-placeholder"></span>
                      <span v-if="!props.disableEditing" class="select-arrow-icon pi pi-chevron-down" @mousedown.stop="handleCellMousedown($event, header.id, item.data.id)" @click.stop="handleArrowClick($event, header.id, item.data.id)"></span>
                    </div>

                    <div v-else-if="header.columnType === 'multi'" class="cell-multi-dir" @mousedown="handleDirCellMousedown(header.id, item.data.id)" @click.stop="handleDirCellClick(header.id, item.data.id)" @dblclick.stop="startCellEdit(header.id, item.data.id)">
                      <template v-if="item.data.cells[header.id].dirValues && item.data.cells[header.id].dirValues.length > 0">
                        <span v-for="(dirValue, idx) in item.data.cells[header.id].dirValues" :key="idx" class="dir-tag" @mouseenter="showDirInfo($event, header, dirValue.dirRowId)" @mouseleave="hideDirInfo">
                          {{ getTagDisplayValue(header, dirValue.dirRowId, dirValue) }}
                          <i v-if="!props.disableEditing" class="pi pi-times dir-tag-remove" @mousedown.stop @click.stop="removeDirTagValue(header, item.data.id, dirValue.dirRowId)"></i>
                        </span>
                      </template>
                      <span v-else class="cell-empty-placeholder"></span>
                    </div>

                    <!-- Issue #6792: Lookup column (Relation) - displays field from related table (read-only) -->
                    <div v-else-if="header.columnType === 'lookup'" class="cell-lookup">
                      <span v-if="getLookupValue(header, item.data)" class="lookup-value">
                        {{ getLookupValue(header, item.data) }}
                      </span>
                      <span v-else class="cell-empty-placeholder"></span>
                    </div>
                  ...
                  <template v-else>
                    <MultiSelect
                      ...
                      overlayClass="integram-table-multiselect"
                      :pt="{ pcChip: { removeIcon: { onClick: (e) => e.stopPropagation() } } }"
                      @keydown="handleMultiSelectKeydown"
                      @show="isDropdownOpen = true"
                      @hide="handleMultiSelectHide"
                      @click="handleDropdownClick"
                      @filter="handleMultiselectFilter"
                    >
                      <template #header>
                        <div class="ref-expand-button-container">
                          <Button
                            icon="pi pi-external-link"
                            label="Развернуть"
                            text
                            size="small"
                            class="ref-expand-button"
                            @click="openReferenceTableModal"
                          />
                          <button
                            type="button"
                            class="p-button p-component p-button-text p-button-sm ref-expand-button multiselect-done-btn"
                            title="Готово"
                            @click.stop="handleCloseMultiSelectEditor"
                          >
                            <span class="pi pi-check p-button-icon"></span>
                          </button>
                        </div>
                      </template>
                      <template #footer v-if="shouldShowAddButton">
                        <div class="ref-add-button-container">
                          <Button
                            :label="`➕ Добавить: «${multiselectFilterText}»`"
                            text
                            class="ref-add-button"
                            @click="handleCreateRefValue"
                          />
                        </div>
                      </template>
                    </MultiSelect>
                    ...
                    <Dropdown
                      v-else-if="currentEditingHeader?.dirTableId"
                      ...
                      :loading="isLoadingDirectory"
                      :disabled="isLoadingDirectory"
                      :placeholder="isLoadingDirectory ? 'Загрузка...' : ''"
                      class="cell-editor seamless-editor"
                      overlayClass="integram-table-dropdown"
                    ...
                    >
                      ...
                      <template #footer v-if="shouldShowAddButton">
                        <div class="ref-add-button-container">
                          <Button
                            :label="`➕ Добавить: «${dropdownFilterText}»`"
                            text
                            class="ref-add-button"
                            @click="handleCreateRefValue"
                          />
                    ...
                    <InputText
                      ...
                      ref="cellEditorInput"
                      v-model="editingValue"
                      @keydown="handleCellEditorKeydown($event, header.id, item.data.id)"
...
import { formatNumber, parseNumberFormat, buildNumberFormatAttrs } from '@/utils/numberFormatter.js'
...
import AIAgentConfigDialog from './DataTable/dialogs/AIAgentConfigDialog.vue'
import SelectColumnSettingsDialog from './DataTable/dialogs/SelectColumnSettingsDialog.vue'
import NumberFormatSettingsDialog from './DataTable/dialogs/NumberFormatSettingsDialog.vue'
...
import { useUndoRedo } from './DataTable/composables/useUndoRedo.js'
...
/**
 * Calculate default column width based on header text length.
 * Assumes text can wrap to 2 lines: width fits ~half the characters per line.
 * ~8px per Cyrillic/Latin char at 13px font + 32px padding/icons.
 * Clamped between 80px and 240px.
 */
...
const BUTTON_ACTIONS = [
  ...
  // === AI Action (legacy - single button) ===
  {
    id: 'ai-action',
    label: 'AI обработка',
    icon: 'pi pi-sparkles',
    actionType: 'ai-action',
    description: 'Обработать строку с помощью AI (настройка модели и промпта)',
    params: ['aiModel', 'aiPrompt', 'outputColumn'],
    paramLabels: {
      aiModel: 'Модель AI (например: deepseek/deepseek-chat)',
      aiPrompt: 'Промпт для AI (можно использовать [VAL] для значения строки)',
      outputColumn: 'ID колонки для результата (необязательно)'
    },
    refreshAction: 'reload-table'
  },

  // === AI Agent Column (Airtable-style - two buttons: configure + execute) ===
  {
    id: 'ai-agent',
    endpoint: 'ai-agent',
    label: 'AI Агент',
    icon: 'pi pi-android',
    actionType: 'ai-agent',
    description: 'Колонка с AI агентом: кнопки Настроить и Выполнить (как в Airtable)',
    params: [],
    refreshAction: 'reload-table'
  },
  ...
  // === Row Operations ===
  {
    ...
    icon: 'pi pi-plus',
    actionType: 'add-row',
    description: 'Создать новую строку в текущей таблице',
    params: [],
    refreshAction: 'reload-table'
  },
  ...
  // === Report Operations ===
  {
    ...
    actionType: 'call-report',
    description: 'Запустить отчет по ID с данными из текущей строки',
    params: ['reportId'],
    paramLabels: { reportId: 'ID отчета' },
    refreshAction: 'reload-table'
  },

  // === Direct API Command (newapi) ===
  {
    id: 'newapi',
    label: 'Прямая команда (newapi)',
    icon: 'pi pi-terminal',
    actionType: 'newapi',
    description: 'Прямой вызов API-функции с произвольными параметрами',
    params: ['apiCommand'],
    paramLabels: { apiCommand: 'Команда (например: _m_set/[ID]?req_123=значение)' }
  }
]
...
const emit = defineEmits([
  ...
  'cell-multi-update',
  'row-delete',
  'row-restore', // Issue #6784: Undo/redo support for row deletion
  'row-move-up',
  'row-duplicate',
  'open-nested',
  'row-change-parent',
  ...
  'ai-agent-configure',
  'ai-agent-execute',
  'ai-agent-save-config',
  'select-column-save-config', // Issue #6838: Save settings for Select/Multiselect columns
  'load-directory-metadata', // Issue #6838/6839: Load directory type metadata for field selection
  'row-expand',
  'add-row-after',
  'column-convert-to-ref',
  'column-convert-to-text',
  'open-add-lookup-column',
  'row-version-history' // Issue #7191: Dataset Versioning — open version history panel for a row
])
...
// AI Agent Config dialog state
const isAIAgentConfigDialogVisible = ref(false)
const aiAgentConfigHeader = ref(null)
const aiAgentConfigRow = ref(null)
const aiAgentCurrentConfig = ref({})

// Select Column Settings Dialog State (Issue #6838)
const isSelectColumnSettingsDialogVisible = ref(false)
const selectColumnConfigHeader = ref(null)
const selectColumnCurrentConfig = ref({})
const selectColumnDirectoryFields = ref([])
const selectColumnDirectoryPreview = ref([])

// Vote Config Dialog State (Issue #6877)
const isVoteConfigDialogVisible = ref(false)
const voteConfigHeader = ref(null)
const voteIconInput = ref('👍')

// Number Format Settings Dialog State (Issue #6867)
const isNumberFormatDialogVisible = ref(false)
...
const referenceTableName = ref('')
const selectedReferenceRow = ref(null)
const referenceTableWrapperRef = ref(null)
...
// Track original column widths for auto-fit toggle (issue #6614)
// Key: headerId, Value: { originalWidth: number, isAutoFit: boolean }
const columnAutoFitState = ref({})
...
const rowCounterWidth = ref(64)  // Increased for Coda.io-style checkbox + actions
...
// Issue #6779: When text wrap is turned OFF — reset all row heights to standard (like Excel)
watch(() => props.textWrapEnabled, (newVal) => {
  if (!newVal) {
    // Clear reactive heights - template will apply standard 36px via inline styles
    rowHeights.value = {}
    saveRowHeights()
  }
})
...
const duplicateHighlightColumn = ref(null) // Column currently highlighted for duplicates
...
// Coda-style Header Menu (Issue #6789)
const hoveredHeaderId = ref(null) // Track which header is being hovered

// Inline + button: JS-based hover tracking (CSS :hover loses state when mouse moves to absolutely-positioned button below tr)
const hoveredRowId = ref(null)
let _hoverLeaveTimer = null

function _rowMouseEnter(rowId) {
  clearTimeout(_hoverLeaveTimer)
  hoveredRowId.value = rowId
}

function _rowMouseLeave() {
  _hoverLeaveTimer = setTimeout(() => { hoveredRowId.value = null }, 80)
}
const headerTieredMenu = ref(null) // TieredMenu ref for column header actions
const currentMenuHeader = ref(null) // Header for which menu is open
...
// Explicitly save and close the multiselect editor (triggered by "Done" button in header)
const handleCloseMultiSelectEditor = () => {
  if (editingCell.value && currentEditingHeader.value?.columnType === 'multi') {
    // Save first (uses current editingMultiValue), then clear state
...
// Issue #6854: Quick create new reference value from dropdown
const dropdownFilterText = ref('')
const multiselectFilterText = ref('')

// Check if we should show "Add new value" button
const shouldShowAddButton = computed(() => {
  const isMulti = currentEditingHeader.value?.columnType === 'multi'
  const filterText = isMulti ? multiselectFilterText.value : dropdownFilterText.value

  if (!filterText || !filterText.trim()) return false
  if (!currentEditingHeader.value?.dirTableId) return false

  // Check if there's an exact match (case-insensitive)
  const hasExactMatch = editingOptions.value.some(
    option => option.value?.toLowerCase() === filterText.trim().toLowerCase()
  )
...
const handleDropdownFilter = (event) => {
  dropdownFilterText.value = event.value || ''
}

const handleMultiselectFilter = (event) => {
  multiselectFilterText.value = event.value || ''
}

// Create new reference value
const handleCreateRefValue = async () => {
  const isMulti = currentEditingHeader.value?.columnType === 'multi'
  const newValue = (isMulti ? multiselectFilterText.value : dropdownFilterText.value).trim()

  if (!newValue) return

  const refTypeId = currentEditingHeader.value?.dirTableId
  if (!refTypeId) return

  emit('create-ref-value', {
    typeId: refTypeId,
    value: newValue,
    isMulti,
    onSuccess: (newObj) => {
      // Issue #6861: Add new value to editingOptions FIRST so it appears in the list
      const newOption = { id: newObj.id, value: newValue }
      if (!editingOptions.value.some(opt => opt.id === newObj.id)) {
        editingOptions.value.push(newOption)
      }

      // Then select the newly created value
      if (isMulti) {
        if (!editingMultiValue.value) {
          editingMultiValue.value = []
        }
        if (!editingMultiValue.value.includes(newObj.id)) {
          editingMultiValue.value.push(newObj.id)
        }
        multiselectFilterText.value = ''
      } else {
        editingValue.value = newObj.id
        dropdownFilterText.value = ''
      }
    }
  })
}

// Open reference table modal (Issue #6859)
const openReferenceTableModal = async () => {
  if (!currentEditingHeader.value?.dirTableId) return

  referenceTableId.value = currentEditingHeader.value.dirTableId
  referenceTableName.value = currentEditingHeader.value.value || 'Справочник'
  selectedReferenceRow.value = null
  isReferenceTableModalVisible.value = true

  // Wait for modal to render, then enable selection mode
  await nextTick()
  if (referenceTableWrapperRef.value) {
    // Access the DataTable inside the wrapper and enable selection mode
    const dataTableRef = referenceTableWrapperRef.value.$refs?.dataTableRef
    if (dataTableRef && typeof dataTableRef.toggleSelectionMode === 'function') {
      const isSelectionActive = dataTableRef.selectionModeEnabled?.() || false
      if (!isSelectionActive) {
        dataTableRef.toggleSelectionMode()
      }
    }
  }
}

// Check if reference selection has any selected rows
const hasReferenceSelection = computed(() => {
  if (!referenceTableWrapperRef.value) return false
  const dataTableRef = referenceTableWrapperRef.value.$refs?.dataTableRef
  if (!dataTableRef) return false
  const selectedRowIds = dataTableRef.selectedRowIds || new Set()
  return selectedRowIds.size > 0
})

// Confirm reference selection and apply to cell
const confirmReferenceSelection = () => {
  if (!referenceTableWrapperRef.value) return
  ...
  // Get the selected row ID(s)
  const selectedIds = Array.from(selectedRowIds)
  const isMulti = currentEditingHeader.value?.columnType === 'multi'

  if (isMulti) {
    // For MultiSelect, add all selected IDs
    if (!editingMultiValue.value) {
      editingMultiValue.value = []
    }
    selectedIds.forEach(id => {
      if (!editingMultiValue.value.includes(id)) {
        editingMultiValue.value.push(id)
      }
    })
  } else {
    // For Dropdown, set the first selected value
    editingValue.value = selectedIds[0]
  }

  // Close the modal
  isReferenceTableModalVisible.value = false

  // Trigger save (same as dropdown change)
  if (editingCell.value && !isMulti) {
    saveCellEdit(editingCell.value.headerId, editingCell.value.rowId)
  }
}
...
// Dir/multi cell: track pre-click selection to detect 2nd click
const preClickSelectedCell = ref(null)

const handleDirCellMousedown = (headerId, rowId) => {
  preClickSelectedCell.value = isCellSelected(headerId, rowId) ? { headerId, rowId } : null
}

const handleDirCellClick = (headerId, rowId) => {
  if (
    preClickSelectedCell.value?.headerId === headerId &&
    preClickSelectedCell.value?.rowId === rowId &&
    !isInlineEditing(headerId, rowId)
  ) {
    startCellEdit(headerId, rowId)
  }
  preClickSelectedCell.value = null
}

// Arrow icon click: always open the dropdown (one-click open via the arrow)
const handleArrowClick = (event, headerId, rowId) => {
  startCellEdit(headerId, rowId)
}

// Remove a single tag from a multi-dir cell without entering edit mode
const removeDirTagValue = (header, rowId, dirRowId) => {
  const currentCell = processedRows.value.find(r => r.id === rowId)?.cells[header.id]
  const currentValues = (currentCell?.dirValues || []).filter(v => v.dirRowId !== dirRowId)
  const overrideKey = `${rowId}:${header.id}`
  // Must store { value, dirValues } object so processedRows updates chips immediately
  const displayString = currentValues
    .map(v => v.displayValue || getTagDisplayValue(header, v.dirRowId, v))
    .join(', ')
  localCellOverrides.value.set(overrideKey, { value: displayString, dirValues: currentValues })
  emit('cell-multi-update', {
    rowId,
    headerId: header.id,
    dirTableId: header.dirTableId,
    dirValues: currentValues
  })
  markCellAsChanged(header.id, rowId)
}

// Clear a single-dir cell value (× button)
const clearDirValue = (header, rowId) => {
  const overrideKey = `${rowId}:${header.id}`
  localCellOverrides.value.set(overrideKey, { value: '', dirRowId: null })
  emit('cell-update', {
    rowId,
    headerId: header.id,
    value: '',
...
// Create command for fill/autofill operation (used at lines ~4528, ~4893)
const createFillCommand = (cells, oldValues) => {
  return {
    type: 'fill',
    data: { cells, oldValues },
    execute: () => cells.forEach(({ headerId, rowId, value }) => emit('cell-update', { headerId, rowId, value })),
    undo: () => oldValues.forEach(({ headerId, rowId, value }) => emit('cell-update', { headerId, rowId, value }))
  }
}
  ...
  props.rows.map(row => ({
    ...
    cells: Object.fromEntries(
      localHeaders.value.map(header => {
        ...
        // localOverride can be a plain string (regular cells) or an object { value, dirRowId, dirValues }
        const overrideIsObject = localOverride !== null && typeof localOverride === 'object'

        const processedCell = {
          ...cell,
          // Use local override if exists, otherwise use original value
          value: overrideIsObject ? localOverride.value : (localOverride !== undefined ? localOverride : cell.value),
          // Spread dir/multi overrides if present (for immediate reactivity after fill/paste)
          ...(overrideIsObject && localOverride.dirRowId !== undefined ? { dirRowId: localOverride.dirRowId } : {}),
          ...(overrideIsObject && localOverride.dirValues !== undefined ? { dirValues: localOverride.dirValues } : {}),
...
// Initialize Undo/Redo composable BEFORE useCellEditing (it passes createCellEditCommand/addToHistory to it)
const {
  historyStack,
  historyIndex,
  canUndo,
...
const {
  ...
  handleMultiSelectKeydown: handleMultiSelectKeydownOriginal,
  handleDropdownKeydown: handleDropdownKeydownOriginal,
  ...
  grantOptions,
  loadGrantOptions,
  // Issue #6784: Pass undo/redo functions for cell edit tracking
  {
    createCellEditCommand,
    addToHistory
  },
  // Issue #6839: Pass getDirectoryOptions for filter-aware inline editing
  getDirectoryOptions,
  // Issue #6839: Pass directoryCache for synchronous filter (avoids async batch load on every click)
  directoryCache
)

// Issue #6854: Wrap keydown handlers to support quick creation with Enter key
const handleDropdownKeydown = (event) => {
  if (event.key === 'Enter' && shouldShowAddButton.value && dropdownFilterText.value.trim()) {
    // Quick create on Enter when no exact match
    event.preventDefault()
    event.stopPropagation()
    handleCreateRefValue()
    return
  }
  // Call original handler
  handleDropdownKeydownOriginal(event)
}

const handleMultiSelectKeydown = (event) => {
  if (event.key === 'Enter' && shouldShowAddButton.value && multiselectFilterText.value.trim()) {
    // Quick create on Enter when no exact match
    event.preventDefault()
    event.stopPropagation()
    handleCreateRefValue()
  ...
  // Call original handler
  handleMultiSelectKeydownOriginal(event)
}
...
const {
  ...
  loadDirectoryList,
  emit,
  // Issue #6784: Pass undo/redo functions for row delete tracking
  {
    createRowDeleteCommand,
    addToHistory
  },
...
// Local row context menu handler that sets row AND shows the context menu
// (The composable's onRowContextMenu only sets contextMenuRow, we need to show it here)
...
// Coda-style TieredMenu items (Issue #6789)
const headerTieredMenuItems = computed(() => {
  if (!currentMenuHeader.value) return []

  const header = currentMenuHeader.value
  const isSorted = isColumnSorted(header.id)
  const hasFilter = hasActiveColumnFilter(header.id)
  const isGrouped = currentGroupColumns.value.includes(header.id)

  const menuItems = []

  // 0. Column info header (ID display + copy)
  menuItems.push({
    label: `${header.value}  ·  ID: ${header.id}`,
    icon: 'pi pi-hashtag',
    class: 'menu-col-id-item',
    command: () => {
      navigator.clipboard?.writeText(String(header.id))
    }
  })
  ...
  if (!props?.disableEditing) {
    menuItems.push({
      ...
      command: () => {
        ...
        emitHeaderAction('rename')
      }
    })
  }

  // 1.5. Settings (for Select/Multiselect columns)
  // Show for columns with refType (dir) or multi columnType
  const isSelectColumn = header.columnType === 'dir' || header.columnType === 'multi'
  if (!props?.disableEditing && isSelectColumn) {
    menuItems.push({
      label: 'Настройки',
      icon: 'pi pi-cog',
      command: () => {
        openColumnSettings(header)
      }
    })
  }

  // 1.6. Number Format Settings (Issue #6867 - for NUMBER columns)
  const isNumberColumn = header.type === 13 || header.type === 14
  if (!props?.disableEditing && isNumberColumn) {
    menuItems.push({
      label: 'Формат числа',
      icon: 'pi pi-hashtag',
      command: () => {
  ...
  if (!props?.disableTypeEditing) {
    menuItems.push({
      label: 'Тип данных',
      icon: 'pi pi-th-large',
      items: types.value.map(type => ({
        label: type.label,
        icon: `fas ${type.icon}`,
        command: () => {
          currentHeader.value = header
          changeColumnType(type.value)
        },
        class: header.type === type.value ? 'menu-item-checked' : ''
      }))
    })
  }
  ...
  const sortItems = [
    {
      ...
      command: () => {
        sortColumns.value = [{ headerId: header.id, direction: 'asc' }]
      }
    },
    {
      label: 'По убыванию ↓',
      ...
      command: () => {
        sortColumns.value = [{ headerId: header.id, direction: 'desc' }]
      }
    }
  ]
  ...
  const filterItems = [
    {
      ...
      icon: 'pi pi-filter',
      command: (e) => toggleColumnFilter(header.id, e.originalEvent)
    }
  ]
  ...
  if (hasFilter) {
    filterItems.push(
      { separator: true },
      {
        ...
        command: () => clearColumnFilter(header.id)
      }
    )
  }
  ...
  // 5. Single-click Group/Ungroup menu item
  menuItems.push({
    label: isGrouped ? 'Убрать группировку' : 'Группировать',
    icon: 'pi pi-objects-column',
    command: () => {
      if (isGrouped) {
        // Remove from group columns
        const newGroupColumns = currentGroupColumns.value.filter(id => id !== header.id)
        toggleMultiGroupBy(newGroupColumns)
      } else {
        // Add to end of group columns (becomes next in order)
        const newGroupColumns = [...currentGroupColumns.value, header.id]
        toggleMultiGroupBy(newGroupColumns)
      }
    }
  })
  ...
  menuItems.push(
    {
      label: pinnedColumns.value.has(header.id) ? 'Открепить колонку' : 'Закрепить колонку',
      icon: pinnedColumns.value.has(header.id) ? 'pi pi-lock-open' : 'pi pi-lock',
      command: () => togglePinColumn(header.id)
    },
    {
      label: 'Скрыть колонку',
      icon: 'pi pi-eye-slash',
      command: () => emit('hide-column', header.id)
    },
    {
      label: 'Показать дубликаты',
      icon: 'pi pi-copy',
      command: () => {
        // Toggle duplicates highlighting for this column
        if (duplicateHighlightColumn.value === header.id) {
          duplicateHighlightColumn.value = null
        } else {
          duplicateHighlightColumn.value = header.id
        }
      }
    }
  )

  // 7. Conversion / Relation
  if (!props?.disableEditing) {
    const isPlainText = header.type === 3 && !header.refType
    const isReference = header.refType != null

    menuItems.push({ separator: true })

    // Always show "Add Relation column" — opens add-column dialog with Lookup type
    menuItems.push({
      label: 'Добавить Relation колонку',
      icon: 'pi pi-link',
    ...
    if (isPlainText) {
      menuItems.push({
        ...
        icon: 'pi pi-sitemap',
        command: () => emit('column-convert-to-ref', header.id)
      })
    }
    ...
    if (isReference) {
      menuItems.push({
        ...
        command: () => emit('column-convert-to-text', header.id)
      })
    }
  }
...
const headerMenuItems = computed(() => [
  ...
  {
    label: groupingMode.value === 'merged'
      ? 'Группировка заголовками'
      : 'Объединить ячейки (Word/Coda)',
    icon: groupingMode.value === 'merged' ? 'pi pi-list' : 'pi pi-th-large',
    visible: currentGroupColumn.value !== null || currentGroupColumns.value.length > 0,
    command: () => toggleGroupingMode()
  },
  { separator: true },
  {
    label: 'Конвертировать в справочник',
  ...
  },
  {
    label: 'Преобразовать в текст',
    icon: 'pi pi-align-left',
    visible: currentHeader.value?.refType != null,
    command: () => emit('column-convert-to-text', currentHeader.value?.id),
    disabled: props?.disableEditing
  },
  ...
  {
    label: 'Настроить иконку голосования',
    icon: 'pi pi-thumbs-up',
    visible: currentHeader.value?.type === 2 && currentHeader.value?.attrs?.includes(':vote:'),
    command: () => openVoteConfigDialog(currentHeader.value),
    disabled: props?.disableEditing
  },
  { separator: true },
  {
    label: pinnedColumns.value.has(currentHeader.value?.id)
      ? 'Открепить колонку'
      : 'Закрепить колонку',
    icon: pinnedColumns.value.has(currentHeader.value?.id) ? 'pi pi-lock-open' : 'pi pi-lock',
...
const rowMenuItems = computed(() => [
  // Row ID display (non-clickable info item)
  {
    label: contextMenuRow.value ? `ID: ${contextMenuRow.value.id}` : 'ID: —',
    icon: 'pi pi-hashtag',
    class: 'row-menu-id-item',
    command: () => copyRowId()
  },
  { separator: true },
  { label: 'Развернуть строку', icon: 'pi pi-expand', command: () => expandRowFromMenu() },
  { label: 'Изменить строку', icon: 'pi pi-pencil', command: () => editRow(), disabled: props?.disableEditing },
  { label: 'История версий', icon: 'pi pi-clock', command: () => openVersionHistory() },
  { separator: true },
  { label: 'Дублировать строку', icon: 'pi pi-copy', command: () => duplicateRow(), disabled: props?.disableEditing },
  ...
  { separator: true },
  { label: 'Скопировать API запрос', icon: 'pi pi-link', command: () => copyApiRequest() },
  { label: 'Копировать значение ячейки', icon: 'pi pi-clone', command: () => copyCellValue() },
...
// Issue #7191: Open version history side panel for the context-menu row
function openVersionHistory() {
  if (contextMenuRow.value) {
    emit('row-version-history', contextMenuRow.value.id)
  }
}

function duplicateRow() {
  if (contextMenuRow.value) {
    emit('row-duplicate', contextMenuRow.value.id)
  }
}

// Coda-style: expand row from context menu
function expandRowFromMenu() {
  if (contextMenuRow.value) {
    emit('row-expand', contextMenuRow.value.id)
  }
}

// Coda-style: expand row directly from hover button
function expandRowDirect(row) {
  emit('row-expand', row.id)
}

// Coda-style: duplicate row directly from hover button
function duplicateRowDirect(row) {
  emit('row-duplicate', row.id)
}

// Coda-style: open actions menu from hover button
function openRowActionsMenu(event, row) {
  contextMenuRow.value = row
  rowContextMenu.value.show(event)
}

// Coda-style: add row after current
function emitAddRowAfter(row) {
  emit('add-row-after', row.id)
}

// Copy row ID to clipboard
function copyRowId() {
  if (contextMenuRow.value) {
    navigator.clipboard.writeText(String(contextMenuRow.value.id)).then(() => {
      // Optionally show a toast or indicator
    }).catch(() => {})
  }
}

// Copy API request for this row to clipboard
function copyApiRequest() {
  if (!contextMenuRow.value) return
  const rowId = contextMenuRow.value.id
  const server = props.serverUrl || window.location.origin
  const db = props.database || 'my'
  const typeId = props.typeId
  let apiUrl = ''
  if (typeId) {
    apiUrl = `${server}/${db}/_d_get/${typeId}?ID=${rowId}`
  } else {
    apiUrl = `${server}/${db}/_d_object/${rowId}`
  }
...
// Issue #6877: Vote icon configuration
function openVoteConfigDialog(header) {
  voteConfigHeader.value = header
  // Parse current icon from attrs: :icon=👍:
  const attrs = header?.attrs || ''
  const iconMatch = attrs.match(/:icon=([^:]+):/)
  voteIconInput.value = iconMatch?.[1] || '👍'
  isVoteConfigDialogVisible.value = true
}

function closeVoteConfigDialog() {
  isVoteConfigDialogVisible.value = false
  voteConfigHeader.value = null
}

async function saveVoteConfig() {
  if (!voteConfigHeader.value) return

  const header = voteConfigHeader.value
  const icon = voteIconInput.value.trim() || '👍'

  // Build new attrs string: :ALIAS=Name:vote:icon=👍:
  const aliasMatch = header.attrs?.match(/:ALIAS=([^:]+):/)
  const alias = aliasMatch?.[1] || header.value || 'Голосование'
  const newAttrs = `:ALIAS=${alias}:vote:icon=${icon}:`

  // Emit event to save attrs
  emit('vote-save-config', {
    headerId: header.id,
...
// Open AI Agent config dialog
function handleAIAgentConfigure(event, rowId, headerId) {
  event.stopPropagation()
  event.preventDefault()
  ...
  const row = props.rows.find(r => String(r.id) === String(rowId))

  // Parse existing config from attrs
  const attrs = header.attrs || ''
  console.log('[handleAIAgentConfigure] Parsing attrs:', attrs)

  let config = { model: 'Kodacode/KodaAgent', prompt: '', outputField: null, useMCP: false } // Issue #6547: Added useMCP

  // Try to extract config from attrs
  const configMatch = attrs.match(/config=(\{[^}]+\})/)
  if (configMatch) {
    try {
      config = { ...config, ...JSON.parse(configMatch[1]) }
    } catch {
      // Invalid JSON, use defaults
    }
  }

  // Parse attrs segments (new format: p=, o= and legacy: aiPrompt, aiModel, outputField)
  const segments = attrs.split(':').filter(s => s.trim())
  for (const segment of segments) {
    if (segment.includes('=')) {
      ...
      // New format
      if (key === 'p') {
        // Unescape colons (%%COL%% -> :) and handle legacy URL-encoded prompts
        let prompt = value
        if (value.includes('%%COL%%')) {
          prompt = value.replace(/%%COL%%/g, ':')
      ...
      if (key === 'o' && value) config.outputField = value
      if (key === 'm') config.useMCP = value === '1' // Issue #6547: Parse MCP mode flag
      // Legacy formats
      if (key === 'aiPrompt') config.prompt = value
      if (key === 'aiModel') config.model = value
      if (key === 'outputField' || key === 'outputColumn') config.outputField = value
    }
  }

  // Convert legacy alias-based outputField to termId
  if (config.outputField && !/^\d+$/.test(config.outputField)) {
    // outputField is an alias, find the termId
    const outputHeader = localHeaders.value.find(h =>
      h.alias === config.outputField || h.value === config.outputField || h.val === config.outputField
    )
    if (outputHeader) {
      config.outputField = String(outputHeader.termId || outputHeader.id?.replace?.(/^req_/, ''))
    }
  }

  console.log('[handleAIAgentConfigure] Parsed config:', config)

  aiAgentConfigHeader.value = header
  aiAgentConfigRow.value = row
  aiAgentCurrentConfig.value = config
  isAIAgentConfigDialogVisible.value = true
}

// Execute AI Agent
function handleAIAgentExecute(event, rowId, headerId) {
  event.stopPropagation()
  event.preventDefault()
  ...
  // Parse config from attrs - model is always Kodacode/KodaAgent
  let config = { model: 'Kodacode/KodaAgent', prompt: '', outputField: null, useWebSearch: false, useMCP: false } // Issue #6547: Added useMCP, Issue #6607: Added useWebSearch

  // Parse attrs segments
  const segments = attrs.split(':').filter(s => s.trim())
  for (const segment of segments) {
    if (segment.includes('=')) {
      const eqIndex = segment.indexOf('=')
      const key = segment.substring(0, eqIndex)
      const value = segment.substring(eqIndex + 1)
      // New format: p=prompt, o=outputField, m=useMCP
      if (key === 'p') {
        // Unescape colons (%%COL%% -> :) and handle legacy URL-encoded prompts
        let prompt = value
        if (value.includes('%%COL%%')) {
      ...
      if (key === 'm') {
        config.useMCP = value === '1' // Issue #6547: Parse MCP mode flag
      }
      if (key === 'w') {
        config.useWebSearch = value === '1' // Issue #6607: Parse web search flag
      }
      // Legacy formats
      if (key === 'aiPrompt') config.prompt = value
      if (key === 'aiModel') config.model = value
  ...
  if (!config.prompt) {
    console.warn('[AI Agent] No prompt configured')
    return
  }

  // Build row context from cells (object) or values (array)
  const rowContext = {}

  if (row?.cells) {
    // cells is an object keyed by header ID
    localHeaders.value.forEach(h => {
      const cellData = row.cells[h.id]
      if (cellData) {
        const name = h.alias || h.value || h.val || h.id
        rowContext[name] = cellData.value
      }
    })
  } else if (row?.values) {
    // values is an array of cell objects
    row.values.forEach(cell => {
      const header = localHeaders.value.find(h => h.id === cell.headerId)
      if (header && cell.value !== undefined) {
        const name = header.alias || header.value || header.val || header.id
        rowContext[name] = cell.value
      }
    })
  }

  // Also add row.id and row main value
  if (row?.id) rowContext['ID'] = row.id
  if (row?.val) rowContext['VAL'] = row.val

  console.log('[handleAIAgentExecute] Built rowContext:', rowContext)

  // Emit execute event
  emit('ai-agent-execute', {
    rowId,
    headerId,
    rowData: rowContext,
    config: config,
...
function handleAIAgentConfigSave(config) {
  if (!aiAgentConfigHeader.value) return

  const header = aiAgentConfigHeader.value

  // Build new attrs - store prompt and outputField (model always Kodacode/KodaAgent)
  // Format: :ALIAS=Label:ai-agent:o=outputField:w=useWebSearch:m=useMCP:p=prompt:
  // IMPORTANT: o=, w=, m= go BEFORE p= because prompt can be long and database may truncate
  // NOTE: Prompt stored as plain Cyrillic (no URL encoding - it makes string 3x longer!)
  // Colons in prompt are escaped as %%COL%%
  const prompt = (config.prompt || '').replace(/:/g, '%%COL%%')
  const outputField = config.outputField || ''
  const useWebSearch = config.useWebSearch ? '1' : '0' // Issue #6607: Save web search flag
  const useMCP = config.useMCP ? '1' : '0' // Issue #6547: Save MCP mode flag
  const newAttrs = `:ALIAS=${header.alias || header.value || 'AI'}:ai-agent:o=${outputField}:w=${useWebSearch}:m=${useMCP}:p=${prompt}:`

  console.log('[handleAIAgentConfigSave] Saving config:', {
    outputField: config.outputField,
    useWebSearch: config.useWebSearch,
    useMCP: config.useMCP,
    prompt: config.prompt?.substring(0, 30) + '...',
    newAttrs: newAttrs.substring(0, 80) + '...'
  })

  // Emit save event to parent
  emit('ai-agent-save-config', {
    headerId: header.id,
    termId: header.termId,
    config: config,
    attrs: newAttrs
  ...
  // Update local header attrs for immediate UI feedback
  const headerIndex = localHeaders.value.findIndex(h => h.id === header.id)
  if (headerIndex !== -1) {
    localHeaders.value[headerIndex].attrs = newAttrs
    console.log('[handleAIAgentConfigSave] Updated local header attrs:', newAttrs)
  }

  closeAIAgentConfigDialog()
}

// Close AI Agent config dialog
function closeAIAgentConfigDialog() {
  isAIAgentConfigDialogVisible.value = false
  aiAgentConfigHeader.value = null
  aiAgentConfigRow.value = null
  aiAgentCurrentConfig.value = {}
}

// Issue #6838: Open Select Column Settings Dialog
async function openColumnSettings(header) {
  selectColumnConfigHeader.value = header

  // Parse existing config from attrs: :ALIAS=Name:df=<reqId>:fc=<expr>:
  const attrs = header.attrs || ''
  const dfMatch = attrs.match(/:df=([^:]+):/)
  const fcMatch = attrs.match(/:fc=([^:]+):/)

  selectColumnCurrentConfig.value = {
    displayField: dfMatch ? dfMatch[1] : null,
    filterCondition: fcMatch ? fcMatch[1].replace(/%%COL%%/g, ':') : ''
  }

  const dirTableId = header.dirTableId
  if (dirTableId) {
    // Ensure directory list is loaded (for preview)
    if (!directoryCache.value[dirTableId]) {
      loadDirectoryList(dirTableId)
    }
    selectColumnDirectoryPreview.value = (directoryCache.value[dirTableId] || []).slice(0, 3)

    // Load directory type metadata to get the list of fields
    emit('load-directory-metadata', {
      dirTableId,
      callback: (metadata) => {
        selectColumnDirectoryFields.value = metadata?.requisites || []
        selectColumnDirectoryPreview.value = (directoryCache.value[dirTableId] || []).slice(0, 3)
      }
    })
  }

  isSelectColumnSettingsDialogVisible.value = true
}

// Issue #6838: Handle Select Column Settings Save
function handleSelectColumnSettingsSave(config) {
  if (!selectColumnConfigHeader.value) return

  const header = selectColumnConfigHeader.value

  // Build attrs: :ALIAS=Label:MULTI:df=<reqId>:fc=<expr>:
  // Colons in filter condition are escaped as %%COL%%
  const displayField = config.displayField || ''
  const filterCondition = (config.filterCondition || '').replace(/:/g, '%%COL%%')
  const isMulti = config.isMulti ?? header.isMulti ?? false

  let newAttrs = `:ALIAS=${header.value || ''}:`
  if (isMulti) newAttrs += `MULTI:`
  if (displayField) newAttrs += `df=${displayField}:`
  if (filterCondition) newAttrs += `fc=${filterCondition}:`

  emit('select-column-save-config', {
    headerId: header.id,
    termId: header.termId,
    config: config,
    attrs: newAttrs,
  ...
  // Update local header immediately for UI feedback
  const headerIndex = localHeaders.value.findIndex(h => h.id === header.id)
  if (headerIndex !== -1) {
    localHeaders.value[headerIndex].attrs = newAttrs
    localHeaders.value[headerIndex].isMulti = isMulti
    localHeaders.value[headerIndex].columnType = isMulti ? 'multi' : 'dir'
    // Also update filterCondition so _needsReqsForDir picks it up on next load
    localHeaders.value[headerIndex].filterCondition = config.filterCondition || ''
  }

  // Invalidate directory cache and immediately reload with withReqs=true if needed.
  // This ensures the filter takes effect right away without requiring a page refresh.
  if (header.dirTableId) {
    invalidateDirectoryCache(header.dirTableId)
    // Now that the header has the new filterCondition, loadDirectoryList will detect
    // via _needsReqsForDir() whether full object data (with reqs) is needed.
    loadDirectoryList(header.dirTableId)
  }

  closeSelectColumnSettingsDialog()
}

// Close Select Column Settings dialog
function closeSelectColumnSettingsDialog() {
  isSelectColumnSettingsDialogVisible.value = false
  selectColumnConfigHeader.value = null
  selectColumnCurrentConfig.value = {}
  selectColumnDirectoryFields.value = []
  selectColumnDirectoryPreview.value = []
}

// Issue #6867: Open Number Format Settings Dialog
function openNumberFormatDialog(header) {
  numberFormatConfigHeader.value = header

  // Parse existing format config from attrs
  const config = parseNumberFormat(header.attrs || '')
  numberFormatCurrentConfig.value = config

  isNumberFormatDialogVisible.value = true
}

// Issue #6867: Handle Number Format Settings Save
function handleNumberFormatSave(config) {
  if (!numberFormatConfigHeader.value) return

  const header = numberFormatConfigHeader.value

  // Build new attrs with number format settings
  const formatAttrs = buildNumberFormatAttrs(config)

  // Preserve ALIAS if it exists
  let newAttrs = ''
  const existingAttrs = header.attrs || ''
  const aliasMatch = existingAttrs.match(/:ALIAS=([^:]+):/)
  if (aliasMatch) {
    newAttrs = `:ALIAS=${aliasMatch[1]}:`
  }
  newAttrs += formatAttrs

  // Emit event to parent to save attrs
  emit('header-action', {
    action: 'update-attrs',
    headerId: header.id,
    termId: header.termId,
  ...
  closeNumberFormatDialog()
}

// Close Number Format Settings dialog
function closeNumberFormatDialog() {
  isNumberFormatDialogVisible.value = false
  numberFormatConfigHeader.value = null
  numberFormatCurrentConfig.value = {}
}
...
function handleTableClick(event) {
  ...
  if (button) {
    // rowId should be converted to number, headerId stays as string (can be "val" or "req_123456")
    const rowId = Number(button.dataset.rowId)
    const headerId = button.dataset.headerId // Keep as string - header IDs use "req_" prefix
    const actionType = button.dataset.actionType

    if (rowId && headerId) {
      // Handle AI Agent specific actions
      if (actionType === 'ai-agent-configure') {
        handleAIAgentConfigure(event, rowId, headerId)
        return
      }
      if (actionType === 'ai-agent-execute') {
        handleAIAgentExecute(event, rowId, headerId)
        return
      }
      // Issue #6877: Handle vote button clicks
      if (actionType === 'vote') {
        handleVoteClick(event, rowId, headerId)
        return
      }
...
async function handleButtonClick(event, rowId, headerId) {
  ...
  try {
    // Special handling for AI action
    if (endpoint === 'ai-action' || actionDef?.actionType === 'ai-action') {
      // Get full row data for AI context
      const rowData = row?.cells || {}
      const rowContext = {}

      // Build context with all column values
      localHeaders.value.forEach(h => {
        const cellData = rowData[h.id]
        if (cellData) {
          rowContext[h.value || h.id] = cellData.value
        }
      })

      // Emit special AI button click event with full context
      emit('ai-button-click', {
        rowId,
        headerId,
        rowData: rowContext,
...
// Issue #6877: Handle vote button clicks with optimistic UI updates
async function handleVoteClick(event, rowId, headerId) {
  event.stopPropagation()
  event.preventDefault()
  ...
  if (!userId) {
    console.warn('No user ID found - cannot vote')
    return
  }
  ...
  let voters = []
  try {
    voters = JSON.parse(currentValue)
  } catch {
    voters = []
  }
  ...
  const newValue = JSON.stringify(voters)

  // Optimistic UI update - update the cell value immediately
  cell.value = newValue
  ...
  if (button) {
    ...
    button.classList.toggle('vote-button-active', hasVoted)
    button.dataset.hasVoted = String(hasVoted)
    button.title = hasVoted ? 'Убрать голос' : 'Голосовать'

    // Update vote count
    const countSpan = button.querySelector('.vote-count')
    if (countSpan) {
      countSpan.textContent = String(voters.length)
    }
  }
  ...
  try {
    ...
    if (button) {
      ...
      const countSpan = button.querySelector('.vote-count')
      if (countSpan) {
        const originalVoters = JSON.parse(currentValue || '[]')
        countSpan.textContent = String(originalVoters.length)
      }
    }
  }
}
...
const formatCellValue = (value, type, rowId, headerId) => {
  ...
  switch (type) {
    // Type 2: HTML or VOTE - Check attrs to determine which
    case 2: {
      // Issue #6877: Vote columns use type 2 (LONG text) with :vote: marker in attrs
      const header = localHeaders.value.find(h => h.id === headerId)
      const attrs = header?.attrs || ''

      // Check if this is a vote column
      if (attrs.includes(':vote:')) {
        // Parse JSON array of voter IDs
        let voters = []
        try {
          voters = JSON.parse(displayValue || '[]')
        } catch {
          voters = []
        }

        // Extract icon from attrs (format: :icon=👍:)
        let icon = '👍' // default
        const iconMatch = attrs.match(/:icon=([^:]+):/)
        ...
        // Render vote button with data attributes for click handler
        const dataAttrs = `data-row-id="${rowId}" data-header-id="${headerId}" data-action-type="vote" data-has-voted="${hasVoted}"`
        const buttonClass = hasVoted ? 'vote-button vote-button-active' : 'vote-button'

        return `<button class="${buttonClass} data-table-action-button" ${dataAttrs} title="${hasVoted ? 'Убрать голос' : 'Голосовать'}"><span class="vote-icon">${icon}</span><span class="vote-count">${voteCount}</span></button>`
      }

      // Regular HTML/text handling
    ...
    // For AI Agent: ":ALIAS=Label:ai-agent:config={...}:"
    case 7: {
      ...
      // For AI Agent: ":ALIAS=Label:ai-agent:config={...JSON...}:"
      let buttonLabel = displayValue || 'Действие'
      let endpoint = ''
      let actionType = 'api-macro'
      let aiConfig = null

      // Extract config/cfg JSON separately (it may contain colons)
      // Supports both :config={...}: and :cfg={...}: formats
      let attrsWithoutConfig = attrs

      // Try both config= (legacy) and cfg= (compact) formats
      let configStartIdx = attrs.indexOf(':cfg=')
      let configKeyLen = 5 // length of ':cfg='
      if (configStartIdx === -1) {
        configStartIdx = attrs.indexOf(':config=')
        configKeyLen = 8 // length of ':config='
      }

      if (configStartIdx !== -1) {
        const jsonStartIdx = configStartIdx + configKeyLen
        if (attrs[jsonStartIdx] === '{') {
          // Balance braces to find end of JSON
          let braceCount = 0
          let jsonEndIdx = jsonStartIdx
          for (let i = jsonStartIdx; i < attrs.length; i++) {
            if (attrs[i] === '{') braceCount++
            else if (attrs[i] === '}') braceCount--
            if (braceCount === 0) {
              jsonEndIdx = i + 1
              break
            }
          ...
          // Only process if we found balanced braces
          if (jsonEndIdx > jsonStartIdx) {
            const configJson = attrs.substring(jsonStartIdx, jsonEndIdx)
            try {
              const parsed = JSON.parse(configJson)
              // Expand compact keys (m, p, o) to full names
              aiConfig = {
                model: parsed.model || parsed.m || 'Kodacode/KodaAgent',
                prompt: parsed.prompt || parsed.p || '',
                outputField: parsed.outputField || parsed.o || null
              }
            } catch {
              // Invalid JSON, ignore
            }
            // Remove config/cfg=... from attrs before splitting
            attrsWithoutConfig = attrs.substring(0, configStartIdx) + attrs.substring(jsonEndIdx)
          } else {
            // Malformed JSON (no closing brace) - remove everything after configStartIdx
            attrsWithoutConfig = attrs.substring(0, configStartIdx)
          }
        }
      }
      const segments = attrsWithoutConfig.split(':').filter(s => s.trim())
      ...
      for (const segment of segments) {
        if (segment.includes('=')) {
          ...
          // New simple format: p=prompt, o=outputField
          if (key === 'p' && value) {
            // Unescape colons (%%COL%% -> :) and handle legacy URL-encoded prompts
            let prompt = value
            if (value.includes('%%COL%%')) {
            ...
            if (!aiConfig) {
              aiConfig = { model: 'Kodacode/KodaAgent', prompt: '', outputField: null }
            }
      ...
      // === AI Agent: Two buttons (Configure + Execute) ===
      // NOTE: Do NOT use inline onclick="event.stopPropagation()" - it prevents event delegation!
      // The handleTableClick function handles events via closest() and handlers do stopPropagation()
      if (actionType === 'ai-agent' || endpoint === 'ai-agent') {
        const hasConfig = aiConfig && aiConfig.prompt
        const configAttrs = `data-row-id="${rowId}" data-header-id="${headerId}" data-action-type="ai-agent-configure"`
        const execAttrs = `data-row-id="${rowId}" data-header-id="${headerId}" data-action-type="ai-agent-execute"`

        const configBtn = `<button class="cell-button cell-button-config data-table-action-button" ${configAttrs} title="Настроить AI"><i class="pi pi-cog"></i></button>`
        const execBtn = hasConfig
          ? `<button class="cell-button cell-button-ai data-table-action-button" ${execAttrs} title="Выполнить AI"><i class="pi pi-play"></i></button>`
          : `<button class="cell-button cell-button-ai-disabled" title="Сначала настройте AI" disabled><i class="pi pi-play"></i></button>`

        return `<div class="ai-agent-cell-buttons">${configBtn}${execBtn}</div>`
      }

      // Render regular button with data attributes for click handler
      ...
      let icon = 'pi-bolt'
      if (isDanger) icon = 'pi-trash'
      else if (actionType === 'ai-action') icon = 'pi-sparkles'
      else if (actionType === 'custom-url') icon = 'pi-external-link'
      else if (actionType === 'add-row') icon = 'pi-plus'
      else if (actionType === 'call-report') icon = 'pi-chart-bar'
      else if (actionType === 'newapi') icon = 'pi-terminal'
    ...
    case 14: { // DECIMAL - число с десятичной частью
      // Issue #6867: Number formatting with various display modes
      ...
      return `<span class="cell-number cell-number-${type === 13 ? 'integer' : 'decimal'}${negativeClass}">${formatted}</span>`
    }
...
function toggleSelectAll() {
  if (isAllRowsSelected.value) {
    // Deselect all - reassign to trigger Vue reactivity
    selectedRowIds.value = new Set()
  } else {
    // Select all visible (filtered) rows - reassign to trigger Vue reactivity
...
function toggleRowSelection(rowId, event, rowIndex) {
  // Work with a new Set to ensure Vue reactivity triggers
  ...
  // Reassign to trigger Vue reactivity (Set mutations alone don't trigger reactivity)
  selectedRowIds.value = newSet
  bulkActionsVisible.value = selectedRowIds.value.size > 0
}
...
function bulkDeleteSelected() {
  ...
  // Emit to parent which handles the confirmation dialog
...
const pinnedColumnsOffsets = computed(() => {
  ...
  localHeaders.value.forEach((header, index) => {
    if (pinnedColumns.value.has(header.id)) {
      ...
      const columnWidth = header.width || calcDefaultColumnWidth(header.value)
...
// Coda-style Header Menu Functions (Issue #6789)
function onHeaderMouseEnter(headerId) {
  hoveredHeaderId.value = headerId
}

function onHeaderMouseLeave() {
  hoveredHeaderId.value = null
}

function openHeaderMenu(event, header) {
  currentMenuHeader.value = header
  if (!headerTieredMenu.value) return

  // Reset target so PrimeVue repositions relative to the current button.
  // PrimeVue TieredMenu uses `this.target = this.target || event.currentTarget`,
  // meaning the target is sticky after first open. Without this reset, the menu
  // always positions relative to the first column button ever clicked, which can
  // result in the menu appearing in the top-left corner if that element is detached.
  headerTieredMenu.value.target = null
  headerTieredMenu.value.show(event)
}

function onHeaderMenuHide() {
  currentMenuHeader.value = null
}
...
async function copySelectedCells() {
  ...
  // Build TSV + structured snapshot (for multi/dir cells)
  const tsvRows = []
  ...
  for (let r = minRow; r <= maxRow; r++) {
    ...
    for (let c = minCol; c <= maxCol; c++) {
      ...
      tsvCells.push(String(cell?.value ?? ''))
      structuredCells.push({
        headerId: header.id,
        columnType: header.columnType,
        dirTableId: header.dirTableId,
        value: cell?.value ?? '',
        dirRowId: cell?.dirRowId ?? null,
        dirValues: cell?.dirValues ? cell.dirValues.map(v => ({ dirRowId: v.dirRowId })) : null
      })
    }
  ...
  // Try to write to system clipboard (may fail without HTTPS/permission)
  try {
    await navigator.clipboard.writeText(tsvData)
  } catch {
    // Internal clipboard is still available
  }
}
...
async function pasteToSelectedCells() {
  ...
  try {
    ...
    // If system clipboard matches internal — use structured data (preserves dir/multi refs)
    if (clipboardData.value) {
      if (!text || text === clipboardData.value.data) {
        text = clipboardData.value.data
        structured = clipboardData.value.structured
      }
    }
    ...
    for (let r = 0; r < rows.length; r++) {
      ...
      for (let c = 0; c < cells.length; c++) {
        ...
        const srcCell = structured?.[r]?.[c]
        const newValue = cells[c]
        const oldValue = row.cells[header.id]?.value ?? ''

        oldValues.push({ rowId: row.id, headerId: header.id, value: oldValue })
        updates.push({ rowId: row.id, headerId: header.id, value: newValue, srcCell })

        // Optimistic UI: store object override for dir/multi so dirRowId/dirValues are immediately reactive
        const ovrKey = `${row.id}:${header.id}`
        if (srcCell?.columnType === 'multi' && srcCell.dirValues) {
          localCellOverrides.value.set(ovrKey, { value: newValue, dirValues: srcCell.dirValues })
        } else if (srcCell?.columnType === 'dir') {
          localCellOverrides.value.set(ovrKey, { value: newValue, dirRowId: srcCell.dirRowId ?? null })
        } else {
          localCellOverrides.value.set(ovrKey, newValue)
        }
        markCellAsChanged(header.id, row.id)
      }
    }

    // Emit updates per column type
    for (const u of updates) {
      const header = localHeaders.value.find(h => h.id === u.headerId)
      if (u.srcCell?.columnType === 'multi' && u.srcCell.dirValues) {
        emit('cell-multi-update', { headerId: u.headerId, rowId: u.rowId, dirTableId: u.srcCell.dirTableId, dirValues: u.srcCell.dirValues })
      } else if (u.srcCell?.columnType === 'dir' && u.srcCell.dirRowId !== null) {
        emit('cell-update', { headerId: u.headerId, rowId: u.rowId, value: u.value, dirRowId: u.srcCell.dirRowId })
      } else {
        emit('cell-update', { headerId: u.headerId, rowId: u.rowId, value: u.value })
      }
    }

    if (updates.length > 0) {
      addToHistory(createFillCommand(updates.map(u => ({ headerId: u.headerId, rowId: u.rowId, value: u.value })), oldValues))
    }
  } catch (err) {
...
// ─── Fill Handle (Excel drag-to-fill) ────────────────────────────────────────

// td mousedown: for dir/multi cells — only select (no startCellEdit on 1st click)
// editing triggered on 2nd click via handleDirCellClick
function handleCellMousedown(event, headerId, rowId) {
  // If a dropdown/multiselect is open on a different cell, save and close it synchronously
  if (isDropdownOpen.value && editingCell.value &&
      (editingCell.value.headerId !== headerId || editingCell.value.rowId !== rowId)) {
    const { headerId: eH, rowId: eR } = editingCell.value
    saveCellEdit(eH, eR)
    cancelCellEdit()
  }

  const header = localHeaders.value.find(h => h.id === headerId)
  if (header?.columnType === 'dir' || header?.columnType === 'multi') {
    if (event.shiftKey && selectedCells.value.start) {
      selectedCells.value = { ...selectedCells.value, end: { headerId, rowId } }
    } else {
      selectedCells.value = { start: { headerId, rowId }, end: null, isSelecting: false }
    }
  } else {
    handleCellClick(event, headerId, rowId)
  }
}

function startFillDrag(event, headerId, rowId) {
  event.preventDefault()
  event.stopPropagation()
  // Prevent dir/multi handleDirCellClick from opening dropdown after fill drag
  preClickSelectedCell.value = null
  fillDragging.value = true
  fillStart.value = { headerId, rowId }
  fillEnd.value = { headerId, rowId }
...
function onFillDragMove(event) {
  ...
  const td = el?.closest('td[data-row-id][data-header-id]')
  // Normalize headerId to string for comparison (dataset values are always strings)
  if (td && td.dataset.headerId === String(fillStart.value.headerId)) {
    fillEnd.value = { headerId: fillStart.value.headerId, rowId: td.dataset.rowId }
  }
}
...
function onFillDragEnd() {
  ...
  if (fillDragging.value && fillStart.value && fillEnd.value &&
      String(fillStart.value.rowId) !== String(fillEnd.value.rowId)) {
    applyFill()
  }
...
function applyFill() {
  const { headerId, rowId: startRowId } = fillStart.value
  const { rowId: endRowId } = fillEnd.value
  // Use string comparison — dataset returns strings, r.id may be number
  const startIdx = processedRows.value.findIndex(r => String(r.id) === String(startRowId))
  const endIdx = processedRows.value.findIndex(r => String(r.id) === String(endRowId))
  if (startIdx < 0 || endIdx < 0 || startIdx === endIdx) return

  const header = localHeaders.value.find(h => h.id === headerId)
  const minIdx = Math.min(startIdx, endIdx)
  const maxIdx = Math.max(startIdx, endIdx)
  const sourceRow = processedRows.value[startIdx]
  const sourceCell = sourceRow?.cells[headerId]
  ...
  for (let i = minIdx; i <= maxIdx; i++) {
    ...
    const row = processedRows.value[i]
    if (!row) continue
    oldValues.push({ headerId, rowId: row.id, value: row.cells[headerId]?.value ?? '' })
    updates.push({ headerId, rowId: row.id, value: sourceValue })
    const key = `${row.id}:${headerId}`
    markCellAsChanged(headerId, row.id)

    if (header?.columnType === 'multi') {
      // Multi: emit cell-multi-update with dirValues array
      const dirValues = (sourceCell?.dirValues || []).map(v => ({ dirRowId: v.dirRowId }))
      // Store object override so reactivity works immediately (dirValues used in template)
      localCellOverrides.value.set(key, { value: sourceValue, dirValues })
      emit('cell-multi-update', { headerId, rowId: row.id, dirTableId: header.dirTableId, dirValues })
    } else if (header?.columnType === 'dir') {
      // Dir: pass dirRowId so wrapper saves reference ID, not display string
      // Store object override so dirRowId is reactive too (used in v-if in template)
      localCellOverrides.value.set(key, { value: sourceValue, dirRowId: sourceCell?.dirRowId ?? null })
      emit('cell-update', { headerId, rowId: row.id, value: sourceValue, dirRowId: sourceCell?.dirRowId })
    } else {
      localCellOverrides.value.set(key, sourceValue)
      emit('cell-update', { headerId, rowId: row.id, value: sourceValue })
    }
  }
  if (updates.length > 0) {
    addToHistory(createFillCommand(updates, oldValues))
  }
}

const isFillTarget = (headerId, rowId) => {
  if (!fillDragging.value || !fillStart.value || !fillEnd.value) return false
  if (String(headerId) !== String(fillStart.value.headerId)) return false
  const startIdx = processedRows.value.findIndex(r => String(r.id) === String(fillStart.value.rowId))
  const endIdx = processedRows.value.findIndex(r => String(r.id) === String(fillEnd.value.rowId))
  const idx = processedRows.value.findIndex(r => String(r.id) === String(rowId))
  const minIdx = Math.min(startIdx, endIdx)
  const maxIdx = Math.max(startIdx, endIdx)
  return idx >= minIdx && idx <= maxIdx && idx !== startIdx
}
...
// Issue #6783: Expand selection in a direction (Shift+Arrow keys)
const expandSelection = (direction) => {
  if (!selectedCells.value.start) return

  // If no end cell, use start cell as the anchor
  const end = selectedCells.value.end || selectedCells.value.start

  const endRowIndex = processedRows.value.findIndex(r => r.id === end.rowId)
  const endColIndex = localHeaders.value.findIndex(h => h.id === end.headerId)

  let newRowIndex = endRowIndex
  let newColIndex = endColIndex

  switch (direction) {
    case 'up':
      newRowIndex = Math.max(0, endRowIndex - 1)
      break
    case 'down':
      newRowIndex = Math.min(processedRows.value.length - 1, endRowIndex + 1)
      break
    case 'left':
      newColIndex = Math.max(0, endColIndex - 1)
      break
    case 'right':
      newColIndex = Math.min(localHeaders.value.length - 1, endColIndex + 1)
      break
  }

  if (newRowIndex !== endRowIndex || newColIndex !== endColIndex) {
    const newRow = processedRows.value[newRowIndex]
    const newHeader = localHeaders.value[newColIndex]

    if (newRow && newHeader) {
      selectedCells.value.end = { headerId: newHeader.id, rowId: newRow.id }
      scrollToCell(newHeader.id, newRow.id)
    }
  }
}

// Issue #6783: Clear content of selected cell(s) - Excel-like Delete behavior
const clearSelectedCellContent = () => {
  ...
  // Collect old values for undo/redo
  const oldValues = []
  ...
  for (let r = minRow; r <= maxRow; r++) {
    ...
    for (let c = minCol; c <= maxCol; c++) {
      ...
      // Skip read-only or special columns
      if (header.id === 'row-counter') continue
  ...
  // Add to history if any cells were cleared
  if (oldValues.length > 0) {
    const cells = oldValues.map(({ headerId, rowId }) => ({ headerId, rowId, value: '' }))
    addToHistory(createFillCommand(cells, oldValues))
  }
}

// Issue #6783: Cut selected cells - Copy and then clear
...
const handleKeyboardNavigation = (event) => {
  // Guard: when user is typing in an input/textarea, let browser handle
  // native text operations (Delete, Backspace, Ctrl+Z/C/V/X) natively.
  const tag = document.activeElement?.tagName
  const isTypingInInput = tag === 'INPUT' || tag === 'TEXTAREA' || document.activeElement?.isContentEditable

  // Issue #6863: When editing a cell, stop propagation of clipboard operations
  // to prevent paste from bubbling up to parent block-editor (Quill), but allow
  // native paste/copy/cut to work in the cell editor itself.
  if (isTypingInInput) {
    if ((event.ctrlKey || event.metaKey) && ['c', 'v', 'x', 'z', 'y'].includes(event.key)) {
      // Stop propagation to prevent parent editor from handling these events
      event.stopPropagation()
      // Do NOT preventDefault - let browser handle natively in the input
    }
    // When focus is inside an input — browser handles everything natively
    return
  }

  // Issue #6783: Undo/Redo shortcuts (Ctrl+Z / Ctrl+Y) — cell-level undo
  ...
  switch (event.key) {
    case 'F2':
      // F2 - start editing with cursor at end (Excel-like)
      if (!props?.disableEditing) {
        event.preventDefault()
        startCellEdit(selectedCells.value.start.headerId, selectedCells.value.start.rowId, { cursorPosition: 'end' })
      }
      break
    case 'Enter':
      ...
      break
    case 'Delete':
      // Delete key on selected cell - clear content (Excel-like)
      if (!props?.disableEditing) {
        event.preventDefault()
        clearSelectedCellContent()
      }
      break
    case 'Backspace':
      // Backspace on selected cell - clear content (Excel-like)
      if (!props?.disableEditing) {
        event.preventDefault()
        clearSelectedCellContent()
      }
    ...
    case 'ArrowRight':
      ...
      break
    case 'Tab':
      // Tab - move to next cell; Issue #6651: auto-create row on Tab at last cell
      event.preventDefault()
      if (!event.shiftKey) {
        // Check if we're on the last column of the last row
        const currentRowIndex = processedRows.value.findIndex(r => r.id === selectedCells.value.start.rowId)
        const currentColIndex = localHeaders.value.findIndex(h => h.id === selectedCells.value.start.headerId)
        const isLastRow = currentRowIndex === processedRows.value.length - 1
        const isLastCol = currentColIndex === localHeaders.value.length - 1
        if (isLastRow && isLastCol && !props.disableEditing) {
          emit('add-row')
        } else {
          navigateToCell('right')
        }
    ...
    case 'PageDown':
      ...
      break
    default:
      // Issue #6783: Typing any printable character on selected cell starts edit in overwrite mode
      if (!props?.disableEditing && event.key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey) {
        event.preventDefault()
...
// Issue #6783: Handle keyboard navigation while editing a cell (Excel-like)
// Enter/Shift+Enter/Tab/Shift+Tab/Arrows save and navigate
const handleCellEditorKeydown = (event, headerId, rowId) => {
  // Don't interfere with Escape (handled separately)
  ...
  if (event.key === 'Tab') {
    ...
    nextTick(() => {
      if (event.shiftKey) {
        ...
        // Check if we're on the last column of the last row
        const currentRowIndex = processedRows.value.findIndex(r => r.id === rowId)
        const currentColIndex = localHeaders.value.findIndex(h => h.id === headerId)
        const isLastRow = currentRowIndex === processedRows.value.length - 1
        const isLastCol = currentColIndex === localHeaders.value.length - 1
        if (isLastRow && isLastCol && !props.disableEditing) {
          emit('add-row')
  ...
  // Handle Enter key (save and move down/up)
  if (event.key === 'Enter') {
    // Don't interfere if Ctrl/Cmd is pressed (might be submit action)
  ...
  // Handle Arrow keys (save and navigate - only if cursor is at edge)
  // This allows normal cursor movement within the input
...
const stopResize = () => {
  ...
  // Issue #6614: Reset auto-fit state when column is manually resized
  // This ensures next double-click will use the new manually set width as "original"
  if (columnAutoFitState.value[headerId]) {
    columnAutoFitState.value[headerId] = {
...
// Issue #6614: Toggle between auto-fit (wide) and original width on subsequent double-clicks
const autoFitColumn = (headerId) => {
  ...
  const currentState = columnAutoFitState.value[headerId]
  const currentWidth = th.offsetWidth

  // If column was auto-fit, restore to original width
  if (currentState?.isAutoFit) {
    const originalWidth = currentState.originalWidth
    applyColumnWidth(th, cells, originalWidth, headerId)
    // Mark as not auto-fit (restored to original)
    columnAutoFitState.value[headerId] = {
  ...
  // Store original width before auto-fit (if not stored yet)
  if (!currentState) {
    columnAutoFitState.value[headerId] = {
      originalWidth: currentWidth,
      isAutoFit: false
    ...
    // Update original width in case it was manually resized
    columnAutoFitState.value[headerId].originalWidth = currentWidth
  }
  ...
  applyColumnWidth(th, cells, optimalWidth, headerId)

  // Mark as auto-fit
  columnAutoFitState.value[headerId].isAutoFit = true
}

// Helper function to apply width to column header and cells
const applyColumnWidth = (th, cells, width, headerId) => {
  ...
  const updatedHeaders = localHeaders.value.map(header =>
    header.id === headerId ? { ...header, width: width } : header
  )
...
const getTagDisplayValue = (header, dirRowId, dirValue = null) => {
  // Issue #6839: If a custom displayField is configured, try to resolve it
  // Priority: dirRowCache (detailed data) → displayValue from API → directoryCache (main value)
  if (header.displayField && header.displayField !== 'val') {
    const cacheKey = getDirRowCacheKey(header.dirTableId, dirRowId)
    const cached = dirRowCache.value.get(cacheKey)
    if (cached?.data?.values) {
      const fieldValue = cached.data.values.find(v => v.headerId === header.displayField)
      if (fieldValue?.value) return fieldValue.value
    }
    // dirRowCache miss — fall through and return default (background preload will populate later)
  }

  // Default: use displayValue from Integram API response
  if (dirValue?.displayValue) {
    return dirValue.displayValue
  }

  // Fallback: directory list cache (only has main value)
  const dirTableId = header.dirTableId
  if (directoryCache.value[dirTableId]) {
...
/**
 * Issue #6792: Get lookup column value
 * Resolves value from related table field based on reference column
 * @param {Object} header - Lookup column header with lookupConfig
 * @param {Object} row - Current row data with cells
 * @returns {string|null} Resolved lookup value or null
 */
const getLookupValue = (header, row) => {
  if (!header.lookupConfig || !row.cells) {
    return null
  }

  const { refReqId, targetReqId } = header.lookupConfig

  // Find the reference column cell in the current row
  const refCell = row.cells[`req_${refReqId}`]
  if (!refCell || !refCell.dirRowId || !refCell.dirTableId) {
    return null
  }

  // Get the referenced row from directory cache
  const dirTableId = refCell.dirTableId
  const dirRowId = refCell.dirRowId

  if (!directoryCache.value[dirTableId]) {
    // Directory not loaded yet - trigger load
    emit('load-directory-list', {
      dirTableId,
      callback: () => {
        // Cache will be updated, component will re-render
      }
    })
    return '...'
  }

  // Find the referenced row in cache
  const referencedRow = directoryCache.value[dirTableId].find(item => item.id === dirRowId)
  if (!referencedRow) {
    return null
  }

  // Get the target field value
  // If targetReqId is 'id', return the ID
  if (targetReqId === 'id') {
    return String(referencedRow.id)
  }

  // Otherwise, look for the field in the referenced row's requisites
  // The directory cache stores simplified data with { id, value, ... }
  // For more complex fields, we might need to fetch full row data
  // For now, return the main value if targetReqId matches
  if (referencedRow.reqs && referencedRow.reqs[targetReqId]) {
    return String(referencedRow.reqs[targetReqId])
  }

  // If we only have basic cache data (id, value), and targetReqId is not 'id',
  // we need to fetch full row data
  // For MVP, return indication that full data is needed
  return referencedRow.value || null
}
...
const preloadDirectories = async () => {
  ...
  // Delegate to useDirectoryCache.preloadAllDirectories which correctly detects
  // whether withReqs=true is needed (for columns with filterCondition referencing field IDs)
  await preloadAllDirectories()
}
...
watch(() => props.headers, (newHeaders) => {
  localHeaders.value = newHeaders.map(header => ({
    ...header,
    width: header.width || calcDefaultColumnWidth(header.value),
...
// Reorder columns when grouping changes - move grouped columns to front
watch(currentGroupColumns, (newGroupColumns, oldGroupColumns) => {
  if (newGroupColumns.length > 0) {
    // Move group columns to front, preserve order of grouped columns
    const nonGrouped = localHeaders.value.filter(h => !newGroupColumns.includes(h.id))
    const grouped = newGroupColumns
      .map(id => localHeaders.value.find(h => h.id === id))
      .filter(Boolean)
    localHeaders.value = [...grouped, ...nonGrouped]
  } else if (oldGroupColumns && oldGroupColumns.length > 0) {
    // When ungrouping, restore original order from props.headers
    const headerOrder = props.headers.map(h => h.id)
    localHeaders.value.sort((a, b) => {
      const indexA = headerOrder.indexOf(a.id)
      const indexB = headerOrder.indexOf(b.id)
...
watch(() => props.rows, (newRows, oldRows) => {
  ...
  // Exception: preserve the override for the cell currently being edited so that
  // a parent re-render triggered by the optimistic update (Bug #1) does not wipe
  // the just-saved optimistic value while the user is still in the same cell.
  // Clearing mid-edit causes the in-progress value to be lost, resulting in only
  // the first character surviving (the watcher fires after each keystroke in
  // FinModel because localOverrides → getPanelRows returns a new array every time).
  if (editingCell.value) {
    const activeKey = `${editingCell.value.rowId}:${editingCell.value.headerId}`
    const activeOverride = localCellOverrides.value.get(activeKey)
    localCellOverrides.value.clear()
    if (activeOverride !== undefined) {
      localCellOverrides.value.set(activeKey, activeOverride)
    }
  } else {
    localCellOverrides.value.clear()
  }
...
// Opening of dir/multi dropdowns is handled by autoOpenDropdown/autoOpenMultiSelect
// (called after options load). No extra click needed here.
...
onBeforeUnmount(() => {
  ...
  // Clear pending directory info timeout
  if (dirInfoTimeout) {
    clearTimeout(dirInfoTimeout)
    dirInfoTimeout = null
  }

  // Clear pending nested preview timeout
  if (nestedPreviewTimeout) {
    clearTimeout(nestedPreviewTimeout)
    nestedPreviewTimeout = null
  }

  // Clear accumulated filter references to allow GC
  columnFilterPopovers.value = {}
})
...
// ── Row Detail Modal (IntegramEnhancedObjectEditor in Dialog) ─────────────────
const rowDetailVisible = ref(false)
const rowDetailObjectId = ref(null)

const openRowDetail = (rowData) => {
  rowDetailObjectId.value = rowData.id
...
td {
  ...
  border: none; /* reset Quill .ql-editor td { border: 1px solid #000 } */
...
.row-counter-cell {
  ...
  position: sticky;
  left: 0;
  overflow: visible; /* allow left-action buttons to extend outside cell boundary */
}
...
/* ── Row counter cell unified content ───────────────────────────────────── */
.cell-content.row-counter-unified {
  position: relative;
  width: 100%;
  height: 100%;
  min-height: 24px;
  overflow: visible;        /* override .cell-content { overflow: hidden } */
  text-overflow: clip;
  white-space: normal;
}

/* Row number: right edge, vertically centred */
.row-number-coda {
  position: absolute;
  right: 5px;
  top: 50%;
...
/* Grip (6 dots): same position as number, hidden by default */
.row-grip-btn {
  display: none;
  position: absolute;
  right: 4px;
...
/* Expand icon: left side inside the counter column */
.row-expand-btn {
  display: none;
  position: absolute;
  left: 0;
...
/* Checkbox: centred, hidden by default, shown only in selection mode */
.row-select-checkbox-coda {
  display: none;
  position: absolute;
  left: 50%;
  top: 50%;
...
/* ── Row hover state ─────────────────────────────────────────────────────── */
tr.data-row:hover .row-counter-unified .row-expand-btn { display: flex; }
tr.data-row:hover .row-counter-unified .row-grip-btn   { display: flex; }
tr.data-row:hover .row-counter-unified .row-number-coda { display: none; }

/* ── Selection mode ──────────────────────────────────────────────────────── */
.row-counter-unified.selection-mode-active .row-select-checkbox-coda { display: flex; }
.row-counter-unified.selection-mode-active .row-number-coda           { display: none; }
.row-counter-unified.row-selected-coda .row-select-checkbox-coda      { display: flex; }
.row-counter-unified.row-selected-coda .row-number-coda               { display: none; }
...
/* Add row button: centered on the bottom border between rows.
   Button is a direct child of td.row-counter-cell (position:sticky = containing block).
   bottom:0 = td's bottom edge. translateY(50%) = shifts down by half button height.
   Result: button center is exactly on the row border, regardless of padding or DPI. */
.row-add-inline-btn {
  display: inline-flex;
  position: absolute;
  bottom: 0;
...
/* Lift counter cell above adjacent rows so inline+ button (at bottom:-7px) is visible */
tr.data-row td.row-counter-cell.hover-active {
  z-index: 3;
}
...
/* Issue #6779: Ensure rows adapt height in wrap mode */
...
/* Wrap OFF: стандартная высота строк как в Excel.
   ВАЖНО: height на <tr> в таблицах = минимум, браузер всё равно
   расширяет по контенту. Поэтому ограничиваем .cell-content (обычный div) —
   он реально слушается height + overflow: hidden. */
...
/* Dir/multi cells: don't clip chip borders */
.text-wrap-disabled .cell-content:has(.cell-dir),
.text-wrap-disabled .cell-content:has(.cell-multi-dir) {
  max-height: none;
  overflow: visible;
}
...
/* Row context menu: ID item style */
:deep(.row-menu-id-item) {
  opacity: 0.7;
  font-size: 0.85rem;
}
:deep(.row-menu-id-item > .p-menuitem-link) {
  cursor: pointer;
}
...
/* Column ID info item in header menu */
:deep(.menu-col-id-item) {
  opacity: 0.75;
  font-size: 0.75rem;
  cursor: default;
}
:deep(.menu-col-id-item:hover) {
  opacity: 1;
  cursor: copy;
}
:deep(.menu-col-id-item .p-menuitem-text) {
  font-family: var(--font-family);
  color: var(--text-color-secondary);
}

/* Column highlight when header menu is open.
   Uses ::before pseudo-elements (outside border-collapse algorithm)
   so border-radius works reliably. */
th.col-menu-active {
  position: relative;
  background: rgba(67, 56, 202, 0.07) !important;
}
th.col-menu-active::before {
  content: '';
  position: absolute;
  inset: 0;
  border-top: 2px solid var(--p-indigo-700, #4338ca);
  border-left: 2px solid var(--p-indigo-700, #4338ca);
  border-right: 2px solid var(--p-indigo-700, #4338ca);
  border-bottom: none;
...
tbody tr:last-child td.col-menu-active::before {
  border-bottom: 2px solid var(--p-indigo-700, #4338ca);
...
/* Fill handle — dot centered at bottom-right corner of selected cell */
.fill-handle {
  position: absolute;
  bottom: -5px;
  right: -5px;
...
/* Fill target preview during drag */
.fill-target {
...
.seamless-editor {
  ...
  height: 100%;
  min-height: 24px;
  /* Prevent MultiSelect from expanding the column width */
  width: 100% !important;
...
/* Issue #6792: Lookup column (Relation) styling */
.cell-lookup {
  height: 100%;
  display: flex;
  align-items: center;
...
.cell-lookup .lookup-value {
  color: var(--p-text-color);
  font-style: italic;
  opacity: 0.9;
}

.cell-lookup .lookup-value::before {
  content: '→ ';
...
.cell-multi-dir {
  ...
  /* Override parent .cell-content's white-space: nowrap so chips can wrap */
  white-space: normal;
  width: 100%;
  max-width: 100%;
}
...
.cell-dir .dir,
.cell-dir .dir-single-tag,
.cell-multi-dir .dir-tag {
  white-space: normal;
  word-break: normal;
  overflow-wrap: break-word;
  max-width: 100%;
...
/* Wrap-disabled: both dir and multi keep left alignment and show one row */
.text-wrap-disabled .cell-content .cell-dir,
.text-wrap-disabled .cell-content .cell-multi-dir {
  flex-wrap: nowrap;
  overflow: hidden;
  justify-content: flex-start;
}

.text-wrap-disabled .cell-content .cell-dir .dir,
.text-wrap-disabled .cell-content .cell-dir .dir-single-tag,
.text-wrap-disabled .cell-content .cell-multi-dir .dir-tag {
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
  max-width: 100%;
...
.dir-single-tag {
  ...
  border: 1px solid var(--p-surface-400, #a1a1aa);
  color: var(--p-text-color);
  background: transparent;
  border-radius: 10px;
  font-size: 0.82em;
...
.app-dark .dir-single-tag:hover {
  ...
  border-color: var(--p-surface-400, #a1a1aa);
}

/* × remove button on multi-dir tags */
.dir-tag-remove {
  font-size: 9px;
  margin-left: 4px;
  opacity: 0;
...
.dir-tag:hover .dir-tag-remove:hover {
  opacity: 1;
  color: var(--p-red-500, #ef4444);
}

/* Small dropdown arrow icon in corner of single-dir cells */
.select-arrow-icon {
  position: absolute;
  top: 50%;
  right: 3px;
...
.cell-dir:hover .select-arrow-icon {
  opacity: 0.5;
  pointer-events: auto;
}
.cell-dir:hover .select-arrow-icon:hover {
  opacity: 1;
}

/* × button to clear a single-dir cell value */
.dir-single-clear {
  font-size: 9px;
  margin-left: 4px;
  opacity: 0;
...
.dir-single-tag:hover .dir-single-clear:hover {
  opacity: 1;
  color: var(--p-red-500, #ef4444);
}

/* Text-wrap mode: editor height should adapt to content, not fill the tall cell */
.text-wrap-enabled .cell-editor.seamless-editor {
  height: auto;
  min-height: 24px;
  top: 4px;
}

/* Seamless editor container: transparent, no border — matches closed cell */
.seamless-editor .p-multiselect,
.seamless-editor :deep(.p-multiselect),
.seamless-editor .p-dropdown,
.seamless-editor :deep(.p-dropdown),
.seamless-editor .p-select,
.seamless-editor :deep(.p-select) {
  width: 100%;
  height: 100%;
  border: none !important;
  ...
  box-shadow: none !important;
  padding: 0 !important;
  /* Left-align content — match view mode (no centering) */
...
/* Chips/tokens in open editor: identical to .dir-tag — size MUST NOT change */
.seamless-editor :deep(.p-multiselect-token),
.seamless-editor :deep(.p-multiselect-chip),
.seamless-editor :deep(.p-multiselect-label .p-chip),
.seamless-editor :deep(.p-chip) {
  display: inline-flex !important;
  align-items: center !important;
  padding: 2px 7px !important;
  margin: 1px 2px 1px 0 !important;
  border: 1px solid var(--p-surface-400, #a1a1aa) !important;
...
/* Token label text: no width constraints — chip size is driven by content like .dir-tag */
.seamless-editor :deep(.p-multiselect-token-label),
.seamless-editor :deep(.p-chip-label) {
  white-space: nowrap;
  overflow: visible;
  min-width: 0;
  flex: 0 0 auto;
...
.seamless-editor :deep(.p-multiselect-chip:hover),
.seamless-editor :deep(.p-multiselect-label .p-chip:hover),
.seamless-editor :deep(.p-chip:hover) {
  background: var(--p-surface-100, #f4f4f5) !important;
  border-color: var(--p-surface-500, #71717a) !important;
}
.app-dark .seamless-editor :deep(.p-multiselect-token),
.app-dark .seamless-editor :deep(.p-multiselect-chip),
.app-dark .seamless-editor :deep(.p-multiselect-label .p-chip),
.app-dark .seamless-editor :deep(.p-chip) {
  border-color: var(--p-surface-500, #71717a) !important;
}
.app-dark .seamless-editor :deep(.p-multiselect-token:hover),
.app-dark .seamless-editor :deep(.p-multiselect-chip:hover),
.app-dark .seamless-editor :deep(.p-chip:hover) {
  background: var(--p-surface-700, #3f3f46) !important;
  border-color: var(--p-surface-400, #a1a1aa) !important;
}
...
/* "Готово" button in multiselect header — icon-only, text variant */
.multiselect-done-btn {
  color: var(--p-primary-color) !important;
  padding: 0.25rem !important;
}

/* Keep chip wrapping stable in multiselect edit mode.
   The overlay is teleported to body (Portal), so overflow:hidden
   on the root element only clips chip content, not the dropdown panel. */
.seamless-editor.p-multiselect {
  height: auto !important;
...
/* Hide native trigger button and clear icon — open via arrow icon or 2nd-click */
.seamless-editor :deep(.p-multiselect-trigger),
.seamless-editor :deep(.p-multiselect-dropdown),
.seamless-editor :deep(.p-dropdown-trigger),
.seamless-editor :deep(.p-select-dropdown),
.seamless-editor :deep(.p-select-clear-icon),
.seamless-editor :deep(.p-dropdown-clear-icon) {
...
/* Dropdown single-value label: exact match to .dir-single-tag style */
.seamless-editor :deep(.p-dropdown-label:not(.p-placeholder)),
.seamless-editor :deep(.p-select-label:not(.p-placeholder)) {
  display: inline-flex !important;
  align-items: center !important;
  flex: none !important;   /* Prevent flex: 1 1 auto from stretching to full width */
...
.app-dark .seamless-editor :deep(.p-dropdown-label:not(.p-placeholder)),
.app-dark .seamless-editor :deep(.p-select-label:not(.p-placeholder)) {
  border-color: var(--p-surface-500, #71717a) !important;
}

/* Dropdown/select label container: no padding, left-aligned */
.seamless-editor :deep(.p-dropdown-label-container),
.seamless-editor :deep(.p-select-label-container) {
  padding: 0 !important;
  display: flex !important;
  align-items: center !important;
  justify-content: flex-start !important;
...
/* Merged cell styles (Word/Coda.io style grouping) */
.merged-cell {
  background-color: var(--surface-card);
...
.merged-cell::after {
  ...
  bottom: 0;
  height: 1px;
  background: var(--p-surface-300, var(--surface-300, #d4d4d4));
}
...
.merged-cell:hover {
  background-color: var(--p-surface-100, var(--surface-100, #f5f5f5));
}
...
.group-button.group-active {
  opacity: 1 !important;
  color: var(--p-primary-color, var(--primary-color)) !important;
}

.group-button.group-active:hover {
  color: var(--p-primary-600, var(--primary-600, #2563eb)) !important;
}
...
.app-dark .row-selected {
  background-color: rgba(var(--primary-500-rgb), 0.1) !important;
}

/* Highlight full row when selected (Coda.io style) */
tr.data-row.row-is-selected td {
  background-color: var(--primary-50) !important;
}

.app-dark tr.data-row.row-is-selected td {
...
/* Multi-level Sorting - Phase 1 Feature Roadmap */
/* Coda-style Header (Issue #6789) - Clean headers with hover menu */
.header-content {
  position: relative;
  display: flex;
  align-items: center;
  ...
  gap: 4px;
  width: 100%;
  /* Max 2 lines: 2 × line-height(1.3) × font-size(13px) ≈ 34px, plus padding from th */
  max-height: calc(2 * 1.3em + 12px);
  overflow: hidden;
  min-height: 36px;
}
...
/* Menu item with checkmark for active type */
:deep(.menu-item-checked) {
  font-weight: 600;
}

:deep(.menu-item-checked::after) {
  content: ' ✓';
  margin-left: auto;
  color: var(--primary-color);
}
...
:deep(.vote-button-active) {
  background: var(--p-primary-50, #eff6ff);
  border-color: var(--p-primary-500, #3b82f6);
  color: var(--p-primary-700, #1d4ed8);
}

:deep(.vote-button-active:hover) {
...
/* === Issue #6867: Number Formatting Styles === */
...
:deep(.progress-bar-fill) {
  height: 100%;
  background-color: var(--p-green-500, #4CAF50);
...
:deep(.cell-stars .star-filled) {
  color: var(--p-yellow-500, #eab308);
  font-size: 14px;
}

:deep(.cell-stars .star-empty) {
  color: var(--p-surface-400, #94a3b8);
  font-size: 14px;
}

/* Number checkbox */
:deep(.cell-number-checkbox) {
  font-size: 18px;
  color: var(--p-text-color, #475569);
}

:deep(.cell-checkbox-checked) {
  color: var(--p-green-600, #16a34a);
}

:deep(.cell-checkbox-unchecked) {
  color: var(--p-surface-400, #94a3b8);
}
...
/* AI Agent Cell Buttons */
:deep(.ai-agent-cell-buttons) {
  display: flex;
  gap: 4px;
  align-items: center;
  justify-content: center;
...
:deep(.cell-button-ai:hover) {
  background: var(--p-green-600);
}

:deep(.cell-button-ai-disabled) {
  background: var(--p-surface-300);
  color: var(--p-text-muted-color);
  cursor: not-allowed;
  padding: 4px 8px;
...
/* AI button dark theme overrides */
.app-dark :deep(.cell-button-config) {
  background: var(--p-surface-700, #334155);
  color: var(--p-surface-100, #f1f5f9);
}
.app-dark :deep(.cell-button-config:hover) {
  background: var(--p-surface-600, #475569);
}
.app-dark :deep(.cell-button-disabled),
.app-dark :deep(.cell-button-ai-disabled) {
  background: var(--p-surface-700, #334155);
  color: var(--p-surface-400, #94a3b8);
}
...
/* Subordinate table button (nested) - fixes bright appearance on dark theme */
.app-dark .cell-nested {
  background: linear-gradient(135deg, var(--p-surface-700) 0%, var(--p-surface-800) 100%);
  border-color: var(--p-surface-600);
}

.app-dark .cell-nested:hover {
  background: linear-gradient(135deg, var(--p-primary-800, rgba(59,130,246,0.2)) 0%, var(--p-primary-900, rgba(59,130,246,0.3)) 100%);
  border-color: var(--p-primary-600, var(--p-primary-color));
}

/* Directory link tags */
.app-dark .dir {
  background: var(--p-indigo-900, rgba(79, 70, 229, 0.2));
  border-color: var(--p-indigo-700, rgba(99, 102, 241, 0.4));
  color: var(--p-indigo-300, #a5b4fc);
}

.app-dark .dir:hover {
  background: var(--p-indigo-800, rgba(79, 70, 229, 0.3));
  border-color: var(--p-indigo-600, rgba(99, 102, 241, 0.6));
}

/* Dark mode overrides already handled above with .app-dark selector */

/* Dark mode: multiselect chips match .dir-tag dark style */
.app-dark .seamless-editor :deep(.p-multiselect-token),
.app-dark .seamless-editor :deep(.p-multiselect-chip),
.app-dark .seamless-editor :deep(.p-multiselect-label .p-chip),
.app-dark .seamless-editor :deep(.p-chip) {
...
.app-dark .seamless-editor .p-multiselect,
.app-dark .seamless-editor :deep(.p-multiselect) {
  background: var(--p-surface-800, #1e293b) !important;
  border-color: var(--p-surface-600, #475569) !important;
}

/* Merged cell styles */
.app-dark .merged-cell {
  background-color: var(--p-surface-800, #1e293b);
  border-right-color: var(--p-surface-600, #475569) !important;
}

.app-dark .merged-cell::after {
  background: var(--p-surface-600, #475569);
}

/* Button action dialog items */
.app-dark .button-action-item {
  background: var(--p-surface-800, #1e293b);
  border-color: var(--p-surface-600, #475569);
}

.app-dark .button-action-item:hover {
  background: rgba(59, 130, 246, 0.1);
  border-color: var(--p-primary-500, #3b82f6);
}

.app-dark .button-action-item.selected {
  background: rgba(59, 130, 246, 0.15);
  border-color: var(--p-primary-500, #3b82f6);
}

.app-dark .button-action-item.danger:hover {
  background: rgba(239, 68, 68, 0.1);
  border-color: var(--p-red-500, #ef4444);
}

.app-dark .button-action-item.danger.selected {
  background: rgba(239, 68, 68, 0.15);
  border-color: var(--p-red-500, #ef4444);
}

.app-dark .button-action-item .action-icon {
  background: var(--p-surface-700, #334155);
  color: var(--p-surface-300, #cbd5e1);
}

.app-dark .button-action-item .action-label {
  color: var(--p-surface-100, #f1f5f9);
}

.app-dark .button-action-item .action-description {
  color: var(--p-surface-400, #94a3b8);
}

/* Cell type overrides for dark theme */
.app-dark :deep(.cell-phone) {
  background: rgba(16, 185, 129, 0.15);
  border-color: rgba(16, 185, 129, 0.3);
  color: var(--p-emerald-300, #6ee7b7);
}

.app-dark :deep(.cell-email) {
  background: rgba(14, 165, 233, 0.15);
  border-color: rgba(14, 165, 233, 0.3);
  color: var(--p-sky-300, #7dd3fc);
}

.app-dark :deep(.cell-number:hover) {
  background-color: var(--p-surface-700, #334155);
}

/* Issue #6867: Dark mode for number formatting */
.app-dark :deep(.cell-currency) {
  color: var(--p-green-400, #4ade80);
}

.app-dark :deep(.cell-percent) {
  color: var(--p-blue-400, #60a5fa);
}

.app-dark :deep(.progress-bar-bg) {
  background: var(--p-surface-700, #334155);
}

.app-dark :deep(.progress-value),
.app-dark :deep(.ring-value) {
  color: var(--p-surface-300, #cbd5e1);
}

.app-dark :deep(.cell-stars .star-empty) {
  color: var(--p-surface-600, #475569);
}

.app-dark :deep(.cell-number-checkbox) {
  color: var(--p-surface-300, #cbd5e1);
}

.app-dark :deep(.cell-checkbox-checked) {
  color: var(--p-green-500, #22c55e);
}

.app-dark :deep(.cell-checkbox-unchecked) {
  color: var(--p-surface-600, #475569);
}

.app-dark :deep(.cell-html) {
  background: var(--p-surface-700, #334155);
  border-color: var(--p-surface-500, #64748b);
}

.app-dark :deep(.cell-grant) {
  background: rgba(168, 85, 247, 0.15);
  border-color: rgba(168, 85, 247, 0.3);
  color: var(--p-purple-300, #d8b4fe);
}

.app-dark :deep(.cell-grant:hover) {
  background: rgba(168, 85, 247, 0.25);
  border-color: rgba(168, 85, 247, 0.5);
}

.app-dark :deep(.cell-grant-editor) {
  background: rgba(245, 158, 11, 0.15);
  border-color: rgba(245, 158, 11, 0.3);
  color: var(--p-amber-300, #fcd34d);
}

.app-dark :deep(.cell-grant-all) {
  background: rgba(59, 130, 246, 0.15);
  border-color: rgba(59, 130, 246, 0.3);
  color: var(--p-blue-300, #93c5fd);
}

.app-dark :deep(.cell-grant-files) {
  background: rgba(20, 184, 166, 0.15);
  border-color: rgba(20, 184, 166, 0.3);
  color: var(--p-teal-300, #5eead4);
}

.app-dark :deep(.cell-password) {
  background: var(--p-surface-700, #334155);
  border-color: var(--p-surface-500, #64748b);
  color: var(--p-surface-400, #94a3b8);
}

.app-dark :deep(.cell-password:hover) {
  background: var(--p-surface-600, #475569);
  border-color: var(--p-surface-400, #94a3b8);
}

.app-dark :deep(.cell-file) {
  background: var(--p-surface-700, #334155);
  border-color: var(--p-surface-500, #64748b);
  color: var(--p-surface-200, #e2e8f0);
}

.app-dark :deep(.cell-file:hover) {
  background: var(--p-surface-600, #475569);
  border-color: var(--p-surface-400, #94a3b8);
}

.app-dark :deep(.cell-boolean-true) {
  background: rgba(34, 197, 94, 0.2);
  color: var(--p-green-400, #4ade80);
}

.app-dark :deep(.cell-boolean-false) {
  background: rgba(239, 68, 68, 0.2);
  color: var(--p-red-400, #f87171);
}

.app-dark :deep(.cell-calculated) {
  background: rgba(249, 115, 22, 0.15);
  border-color: rgba(249, 115, 22, 0.3);
  color: var(--p-orange-300, #fdba74);
}

.app-dark :deep(.cell-calculated:hover) {
  background: rgba(249, 115, 22, 0.25);
  border-color: rgba(249, 115, 22, 0.5);
}

.app-dark :deep(.cell-report-col) {
  background: rgba(6, 182, 212, 0.15);
  border-color: rgba(6, 182, 212, 0.3);
  color: var(--p-cyan-300, #67e8f9);
}

.app-dark :deep(.cell-report-col:hover) {
  background: rgba(6, 182, 212, 0.25);
  border-color: rgba(6, 182, 212, 0.5);
}

.app-dark :deep(.cell-path) {
  background: var(--p-surface-700, #334155);
  border-color: var(--p-surface-500, #64748b);
  color: var(--p-surface-300, #cbd5e1);
}

.app-dark :deep(.cell-path:hover) {
  background: var(--p-surface-600, #475569);
}

.app-dark :deep(.cell-path-link) {
  background: rgba(59, 130, 246, 0.15);
  border-color: rgba(59, 130, 246, 0.3);
  color: var(--p-blue-300, #93c5fd);
}

.app-dark :deep(.cell-path-link:hover) {
  background: rgba(59, 130, 246, 0.25);
  border-color: rgba(59, 130, 246, 0.5);
  color: var(--p-blue-200, #bfdbfe);
}

.app-dark :deep(.cell-file-fallback) {
  background: rgba(99, 102, 241, 0.15);
  border-color: rgba(99, 102, 241, 0.3);
  color: var(--p-indigo-300, #a5b4fc);
}

/* Date/datetime chip styles for dark theme */
.app-dark :deep(.cell-date),
.app-dark :deep(.cell-datetime) {
  background: var(--p-surface-700, #334155);
  border-color: var(--p-surface-500, #64748b);
}

.app-dark :deep(.cell-date:hover),
.app-dark :deep(.cell-datetime:hover) {
  background: var(--p-surface-600, #475569);
}

.app-dark :deep(.cell-date.date-today),
.app-dark :deep(.cell-datetime.date-today) {
  background: rgba(34, 197, 94, 0.15);
  border-color: rgba(34, 197, 94, 0.3);
  color: var(--p-green-300, #86efac);
}

.app-dark :deep(.cell-date.date-future),
.app-dark :deep(.cell-datetime.date-future) {
  background: rgba(59, 130, 246, 0.15);
  border-color: rgba(59, 130, 246, 0.3);
  color: var(--p-blue-300, #93c5fd);
}

.app-dark :deep(.cell-date.date-far-future),
.app-dark :deep(.cell-datetime.date-far-future) {
  background: rgba(6, 182, 212, 0.15);
  border-color: rgba(6, 182, 212, 0.3);
  color: var(--p-cyan-300, #67e8f9);
}
...
/* Date badges */
.app-dark :deep(.date-badge) {
  background: var(--p-surface-600, #475569);
  color: var(--p-surface-200, #e2e8f0);
}

.app-dark :deep(.date-badge.date-today) {
  background: rgba(34, 197, 94, 0.2);
  color: var(--p-green-300, #86efac);
}

.app-dark :deep(.date-badge.date-past) {
  background: var(--p-surface-700, #334155);
  color: var(--p-surface-400, #94a3b8);
}

.app-dark :deep(.date-badge.date-future) {
  background: rgba(59, 130, 246, 0.2);
  color: var(--p-blue-300, #93c5fd);
}

.app-dark :deep(.date-badge.date-far-past) {
  background: var(--p-surface-800, #1e293b);
  color: var(--p-surface-500, #64748b);
}

.app-dark :deep(.date-badge.date-far-future) {
  background: rgba(6, 182, 212, 0.2);
  color: var(--p-cyan-300, #67e8f9);
}

/* Date pill styles (used for nested/subordinate date display) */
.app-dark :deep(.date-pill) {
  background: linear-gradient(135deg, var(--p-surface-700) 0%, var(--p-surface-800) 100%);
  border-color: var(--p-surface-600);
}

.app-dark :deep(.date-pill:hover) {
  background: linear-gradient(135deg, rgba(59,130,246,0.2) 0%, rgba(59,130,246,0.3) 100%);
  border-color: rgba(59, 130, 246, 0.4);
}

.app-dark :deep(.date-pill.date-today) {
  background: linear-gradient(135deg, rgba(34,197,94,0.15) 0%, rgba(34,197,94,0.25) 100%);
  border-color: rgba(34, 197, 94, 0.3);
  color: var(--p-green-300, #86efac);
}

.app-dark :deep(.date-pill.date-past),
.app-dark :deep(.date-pill.date-far-past) {
  background: linear-gradient(135deg, var(--p-surface-700) 0%, var(--p-surface-800) 100%);
  border-color: var(--p-surface-600);
}

.app-dark :deep(.date-pill.date-future) {
  background: linear-gradient(135deg, rgba(59,130,246,0.15) 0%, rgba(59,130,246,0.25) 100%);
  border-color: rgba(59, 130, 246, 0.3);
  color: var(--p-blue-300, #93c5fd);
}

.app-dark :deep(.date-pill.date-far-future) {
...
/* Date outline style */
.app-dark :deep(.date-outline) {
  border-color: var(--p-surface-500, #64748b);
  color: var(--p-surface-300, #cbd5e1);
}

.app-dark :deep(.date-outline:hover) {
  background: var(--p-surface-700, #334155);
}

.app-dark :deep(.date-outline.date-today) {
  border-color: rgba(34, 197, 94, 0.5);
  color: var(--p-green-300, #86efac);
  background: rgba(34, 197, 94, 0.1);
}

.app-dark :deep(.date-outline.date-future) {
  border-color: rgba(59, 130, 246, 0.5);
  color: var(--p-blue-300, #93c5fd);
  background: rgba(59, 130, 246, 0.1);
}

.app-dark :deep(.date-outline.date-far-future) {
  border-color: rgba(6, 182, 212, 0.5);
  color: var(--p-cyan-300, #67e8f9);
...
/* Date tag style */
.app-dark :deep(.date-tag.date-past) {
  background: var(--p-surface-600, #475569);
  color: var(--p-surface-300, #cbd5e1);
}

.app-dark :deep(.date-tag.date-far-past) {
  background: var(--p-surface-700, #334155);
  color: var(--p-surface-400, #94a3b8);
}
...
.app-dark :deep(.date-calendar-time) {
  color: var(--p-surface-400, #94a3b8);
  border-top-color: var(--p-surface-600, #475569);
}

/* Date dir (directory with date) */
.app-dark :deep(.date-dir) {
  background: linear-gradient(135deg, var(--p-surface-700) 0%, var(--p-surface-800) 100%);
  border-color: var(--p-surface-600);
}

.app-dark :deep(.date-dir.date-today) {
  background: linear-gradient(135deg, rgba(34,197,94,0.15) 0%, rgba(34,197,94,0.25) 100%);
  border-color: rgba(34, 197, 94, 0.3);
  color: var(--p-green-300, #86efac);
}

.app-dark :deep(.date-dir.date-past),
.app-dark :deep(.date-dir.date-far-past) {
  background: linear-gradient(135deg, var(--p-surface-700) 0%, var(--p-surface-800) 100%);
}

.app-dark :deep(.date-dir.date-future) {
  background: linear-gradient(135deg, rgba(59,130,246,0.15) 0%, rgba(59,130,246,0.25) 100%);
  border-color: rgba(59, 130, 246, 0.3);
  color: var(--p-blue-300, #93c5fd);
}

.app-dark :deep(.date-dir.date-far-future) {
  background: linear-gradient(135deg, rgba(6,182,212,0.15) 0%, rgba(6,182,212,0.25) 100%);
  border-color: rgba(6, 182, 212, 0.3);
  color: var(--p-cyan-300, #67e8f9);
}

/* Date modern style text colors */
.app-dark :deep(.date-modern.date-today) {
  color: var(--p-green-300, #86efac);
}

.app-dark :deep(.date-modern.date-future) {
  color: var(--p-blue-300, #93c5fd);
}

.app-dark :deep(.date-modern.date-far-future) {
...
.app-dark .add-column-btn:hover:not(:disabled) {
  background: var(--p-surface-700, #334155);
}

/* File editor hover */
.app-dark .file-editor-seamless .file-editor-label:hover {
  background: var(--p-surface-700, #334155);
}

.app-dark .file-editor-seamless .file-editor-clear:hover {
  background: rgba(239, 68, 68, 0.15);
}
...
.app-dark .footer-cell,
.app-dark .row-counter-footer {
  background: var(--p-surface-800, #1e293b);
}

.app-dark .footer-cell:hover {
  background: var(--p-surface-700, #334155);
}

/* Merged cell hover */
.app-dark .merged-cell:hover {
  background-color: var(--p-surface-700, #334155);
}

/* Button action item icon background */
.app-dark .button-action-item .action-icon {
  background: var(--p-surface-700, #334155);
}

/* Cell file thumbnail border */
.app-dark :deep(.cell-file-thumbnail) {
  border-color: var(--p-surface-600, #475569);
}

/* Cell file fallback (indigo) */
.app-dark :deep(.cell-file-fallback) {
  background: rgba(99, 102, 241, 0.15);
  border-color: rgba(99, 102, 241, 0.3);
  color: var(--p-indigo-300, #a5b4fc);
...
<!-- Global styles for teleported dropdown/multiselect panels (appended to <body>) -->
<style>
/* Integram table: dropdown overlay — word wrap + fixed width */
.integram-table-dropdown,
.integram-table-multiselect {
  max-width: 400px !important;
  min-width: 160px !important;
}

/* Items in dropdown: wrap by words only */
.integram-table-dropdown .p-select-option,
.integram-table-dropdown .p-dropdown-item,
.integram-table-dropdown li {
  white-space: normal !important;
...
.integram-table-multiselect .p-multiselect-option,
.integram-table-multiselect .p-multiselect-item,
.integram-table-multiselect li {
  white-space: normal !important;
  word-break: normal !important;
  overflow-wrap: break-word !important;
  line-height: 1.4;
...
/* Issue #6854: Add new reference value button in dropdown/multiselect footer */
.ref-add-button-container {
  border-top: 1px solid var(--surface-border);
  padding: 0.5rem;
}
...
/* Issue #6859: Expand button in dropdown/multiselect header */
.ref-expand-button-container {
  border-bottom: 1px solid var(--surface-border);
  padding: 0.5rem;
  display: flex;
  justify-content: space-between;
...
.ref-expand-button:hover {
  background-color: var(--primary-50) !important;
}

/* Reference table modal styling */
.reference-table-modal :deep(.p-dialog-content) {
  padding: 0;
  overflow: hidden;
}

/* Issue #6877: Vote config dialog styles */
.vote-config-form {
  padding: 12px 0;
}

.vote-config-form .form-field {
  margin-bottom: 16px;
}

.vote-config-form label {
  display: block;
  margin-bottom: 8px;
  font-weight: 500;
  color: var(--text-color);
