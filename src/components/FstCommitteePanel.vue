...
<script setup>
import { ref, computed, reactive, nextTick, onUnmounted, watch } from 'vue'
import Button from 'primevue/button'
import Select from 'primevue/select'
import {
  AGENTS, SCORING_DIMS, PHASES, PHASE_ORDER, VERDICTS,
  PROJECTS_POOL, SUBFUNDS,
...
const projectOptions = computed(() =>
  PROJECTS_POOL.map(p => ({
    label: `${p.title} (${(p.requestedAmount / 1e6).toFixed(0)} млн)`,
    value: p.id,
  }))
)
...
function scoreColor(s) {
  return s >= 72 ? '#4caf50' : s >= 50 ? '#ffa726' : '#ef5350'
}
