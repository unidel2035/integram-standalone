...
// Use getter function to support auto-detection (Issue #6346)
const getMCPBaseURL = () => getOrchestratorUrlEnv()
const getMCPIntegramEndpoint = () => `${getMCPBaseURL()}/api/mcp/integram`
...
class IntegraMCPService {
  ...
  async authenticate({ serverURL, database, login, password }) {
    try {
      const response = await axios.post(`${getMCPIntegramEndpoint()}/execute`, {
  ...
  async getAvailableTools() {
    try {
      const response = await axios.get(`${getMCPIntegramEndpoint()}/tools`)
      if (response.data.success) {
