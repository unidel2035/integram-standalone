...
class IntegramChatSessionService {
  ...
  async createMessageTransaction(data) {
    ...
    try {
      ...
      // API может вернуть id, obj, или Array [{id, ...}]
      const txData = Array.isArray(result) ? result[0] : result
      const transactionId = txData?.id || txData?.obj
      ...
      if (result && transactionId) {
        ...
        console.warn('[IntegramChatSession] Unexpected createObject response:', JSON.stringify(result))
        return { success: false, error: 'Unexpected response from Integram API' }
      }
    } catch (error) {
      console.warn('[IntegramChatSession] Failed to create transaction:', error.message)
      return { success: false, error: error.message }
    }
  }
...
export { IntegramChatSessionService }
