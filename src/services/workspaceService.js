...
export async function createWorkspace(userId, options = {}) {
  ...
  if (!integramServer) {
    try {
      // Dynamically import integramApiClient to avoid circular dependencies
      const { default: integramService } = await import('./integramApiClient');
...
export async function getGitStatus(workspaceId) {
  const response = await fetch(`${BASE_URL}/workspace-chat/workspaces/${workspaceId}/git/status`)
  if (!response.ok) throw new Error('Git status failed')
  return response.json()
}

export async function gitPull(workspaceId) {
  const response = await fetch(`${BASE_URL}/workspace-chat/workspaces/${workspaceId}/git/pull`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}'
  })
  if (!response.ok) throw new Error('Git pull failed')
  return response.json()
}

export async function gitPush(workspaceId) {
  const response = await fetch(`${BASE_URL}/workspace-chat/workspaces/${workspaceId}/git/push`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}'
  })
...
export async function gitCommit(workspaceId, message, addAll = false) {
  const response = await fetch(`${BASE_URL}/workspace-chat/workspaces/${workspaceId}/git/commit`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ message, addAll })
  })
  if (!response.ok) throw new Error('Git commit failed')
  return response.json()
}

export async function searchWorkspace(workspaceId, query) {
  const response = await fetch(`${BASE_URL}/workspace-chat/workspaces/${workspaceId}/search?q=${encodeURIComponent(query)}`)
