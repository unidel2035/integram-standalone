...
class IntegramApiClient {
  constructor() {
    ...
    // Flag to prevent multiple concurrent redirects to login on auth failure
  ...
  /**
   * Get current auth token (Issue #6660)
   */
  getToken() {
    return this.token
  }
  ...
  /**
   * Get current XSRF token (Issue #6660)
   */
  getXsrfToken() {
    ...
    try {
      ...
      if (stored) {
        ...
        if (sessionData.version === 2 && sessionData.databases) {
          ...
          // CRITICAL: Always refresh databases['my'].token from fresh my_token in localStorage.
          // The token stored in integram_session may be stale (user re-authenticated on main site).
          // loadSessionFromMyToken() updates databases['my'].token = localStorage.my_token
          // WITHOUT overwriting this.database (preserves currentDatabase).
   ...
   * Issue #6591: Also allow using shared auth from /auth/login for all databases
   */
  loadSessionFromMyToken() {
    try {
      ...
      if (myToken && myXsrf) {
        // Issue #6474: DO NOT overwrite this.database - preserve target database
        if (!this.database) {
          this.database = 'my'
        }
        this.token = myToken
        ...
        // Issue #6583: Populate databases['my'] so getAuthHeaders() can find it.
        // Always update the token/xsrf from localStorage (may have changed if user
        // re-authenticated on the main site since the last integram_session was saved).
        if (!this.databases['my']) {
          ...
          // Sync fresh tokens so getAuthHeaders() doesn't use a stale cached token
          this.databases['my'].token = myToken
          this.databases['my'].xsrfToken = myXsrf
          // Issue #6967: Also sync userName so switchDatabase can pass userLogin to backend
          if (myUser && !this.databases['my'].userName) {
            this.databases['my'].userName = myUser
          }
          if (myUserId && !this.databases['my'].userId) {
            this.databases['my'].userId = myUserId
          }
        }

        console.log('[integramApiClient] Loaded session from my_token, target db:', this.database)
        return true
      }

      // Issue #6591: Try legacy token keys (token, _xsrf) from shared /auth/login
      const legacyToken = localStorage.getItem('token')
      const legacyXsrf = localStorage.getItem('_xsrf')
      const legacyUser = localStorage.getItem('user')
      const legacyUserId = localStorage.getItem('id')

      if (legacyToken && legacyXsrf) {
        // Issue #6474: DO NOT overwrite this.database - preserve target database
        if (!this.database) {
          this.database = currentDb || 'my'
        }
        this.token = legacyToken
        this.xsrfToken = legacyXsrf
        this.userId = legacyUserId
        this.userName = legacyUser
        this.authDatabase = currentDb || 'my'

        // Issue #6583: Populate databases['my'] so getAuthHeaders() sends 'my' header
        // for cross-database access (e.g. kval via my token)
        if (this.authDatabase === 'my' && !this.databases['my']) {
          this.databases['my'] = {
            token: legacyToken,
        ...
        console.log('[integramApiClient] Loaded session from shared /auth/login, target db:', this.database, 'authDb:', this.authDatabase)
        return true
      }
  ...
  async validateSession() {
    ...
    // Validate against the auth database ('my'), not the current target database.
    // Validating via the target sub-database can fail with 401 if:
    //  - the saved databases['tech'].token is stale (different from current my_token)
    //  - the target DB token and my_token diverged due to re-login on main site
    // Using 'my' is always correct since it is the original issuer of all tokens.
    //
    // IMPORTANT: We do NOT change this.database here — changing shared state during
    // an async operation would interfere with concurrent getDictionary() / loadTables()
    // calls that are also running in parallel.
    const authDb = this.databases['my']?.token ? 'my' : (this.authDatabase || 'my')
    const cleanBaseURL = this.baseURL.replace(/\/$/, '')
    const url = `${cleanBaseURL}/${authDb}/xsrf`
    const headers = this.getAuthHeaders(authDb)
    ...
    try {
      ...
      if (data.token) {
        ...
        // Sync token into databases['my'] so cross-db requests stay fresh
        if (this.databases['my']) {
          this.databases['my'].token = data.token
        }
      }
      if (data._xsrf) {
        this.xsrfToken = data._xsrf
        if (this.databases['my']) {
          this.databases['my'].xsrfToken = data._xsrf
        }
      }
      if (data.id) {
        this.userId = data.id
      ...
      // Save refreshed session (this.database is still the original target DB — correct)
      this.saveSession()
      return true
    } catch (error) {
   ...
   * Issue #6474: Preserve current target database when restoring session
  ...
  tryRestoreSession() {
    ...
    // Issue #6474: Remember current target database before restoration
    const currentTargetDb = this.database
    ...
    if (stored) {
      try {
        ...
        if (sessionData.token && sessionData.xsrfToken) {
          // Issue #6474: Preserve target database if already set
          // Only use stored database if no target is set
          if (!currentTargetDb) {
            this.database = sessionData.database
          }
          this.token = sessionData.token
          ...
          const authDb = this.authDatabase
          if (authDb && !this.databases[authDb]) {
            this.databases[authDb] = {
              token: sessionData.token,
              xsrfToken: sessionData.xsrfToken,
          ...
          console.log('[integramApiClient] Session restored from localStorage, target db:', this.database, 'authDb:', authDb)
          return true
        }
      } catch (e) {
  ...
  buildURL(endpoint) {
    ...
    // to avoid CORS errors (e.g. kval-proxy for embedded kval tables)
    if (this.proxyPath) {
      const cleanProxy = this.proxyPath.replace(/\/$/, '')
      return `${cleanProxy}/${endpoint}`
    }
  ...
  async getOwnedDatabases(userId) {
    try {
      // CRITICAL: Do NOT modify this.database — doing so causes a race condition
      // where concurrent getDictionary() calls (e.g. for kval) get 401 and on retry
      // pick up this.database='my' and go to my/dict instead of kval/dict.
      //
      // Instead, build the URL explicitly and call axios directly, bypassing get().
      const cleanBaseURL = this.baseURL.replace(/\/$/, '')
      const url = `${cleanBaseURL}/my/object/271/`
      const headers = this.getAuthHeaders('my')

      const axiosResponse = await axios.get(url, {
        params: { JSON_KV: '', F_U: userId },
        headers,
        timeout: 30000
      })
      const response = axiosResponse.data

      // Parse response to extract database names from "БД" reference field (req 1000000839)
      const databases = []

      if (response && response.object && Array.isArray(response.object)) {
        // Response structure: { object: [...], reqs: { objId: { "1000000839": "dbname", ... } } }
        // Database name is in requisite 1000000839 "БД" (reference to lookup table 1000000837)
        const reqs = response.reqs || {}

        for (const obj of response.object) {
          const objReqs = reqs[obj.id] || {}
          // Read from "БД" reference field — Integram returns display name of referenced object
          const dbName = objReqs['1000000839']
  ...
  /**
   * Ensure we have a valid 'my' session for DDL operations
   * DDL operations (_d_*) require 'my' token in 'my' header
   * @returns {Promise<boolean>} True if 'my' session is available
   */
  async ensureMySession() {
    if (this.databases['my']?.token) {
      return true
    }

    // Try to authenticate to 'my' with system credentials
    try {
      console.log('[integramApiClient] Authenticating to "my" for DDL access...')

      // Save current state
      const originalDatabase = this.database
      const originalToken = this.token
      const originalXsrf = this.xsrfToken
      ...
      if (response.data?.token) {
        ...
        this.xsrfToken = originalXsrf

        this.saveSession()
        console.log('[integramApiClient] "my" session established for DDL')
        return true
      }

      return false
    } catch (error) {
      console.error('[integramApiClient] Failed to establish "my" session:', error.message)
      return false
    }
  }
  ...
  async switchDatabase(database) {
    try {
      ...
      const myToken = mySession?.token || this.token || localStorage.getItem('my_token')

      // For 'my' database — simple activation, no exchange needed
      if (database === 'my') {
        if (!myToken) throw new Error('No session. Please authenticate first.')
        this.currentDatabase = 'my'
        this.database = 'my'
        this.token = myToken
        this.xsrfToken = mySession?.xsrfToken || this.xsrfToken
        this.userId = mySession?.userId || this.userId
        this.userName = mySession?.userName || this.userName
        this.userRole = mySession?.userRole || this.userRole
        this.authDatabase = 'my'
        this.saveSession()
        return true
      }

      // For sub-databases: check if we already have a valid cached session.
      // Issue #6967: Only use cache when the cached token IS the user's own myToken
      // (meaning the backend previously confirmed myToken works in this db via retry-after-newDb).
      // If the cached token differs from myToken, it's a cookie-exchange system token —
      // always re-fetch so the backend can try returning the real user's token.
      const cached = this.databases[database]
      if (cached?.token && cached.token === myToken) {
        // Cached user token — the user's own myToken works in this db
        this.currentDatabase = database
        this.database = database
        this.token = cached.token
        this.xsrfToken = cached.xsrfToken || mySession?.xsrfToken
        this.userId = cached.userId || mySession?.userId
        this.userName = cached.userName || mySession?.userName
        this.userRole = cached.userRole || mySession?.userRole
        this.authDatabase = database
        this.saveSession()
        console.log(`[integramApiClient] switchDatabase('${database}'): using cached user token`)
        return true
      }
      ...
      // Call backend switch-db to obtain a valid token for this sub-database.
      // Backend Strategy 1: X-Authorization: myToken → sub-db/xsrf (fast, no exchange)
      // Backend Strategy 2: direct sys-cred auth to sub-db
      // Backend Strategy 3: Cookie: my={myToken} → PHP cookie exchange
      // Issue #6967: Pass the real user login so the backend can authenticate as the correct user.
      // The myToken may belong to the system user 'd', not the logged-in user (e.g. 'korshunov').
      const realUserLogin = mySession?.userName
        || this.databases?.['my']?.userName
        || localStorage.getItem('integram_user')
        || localStorage.getItem('my_user')
        || null
      console.log(`[integramApiClient] switchDatabase('${database}'): calling backend switch-db, realUser=${realUserLogin}`)
      const serverHost = this.baseURL.replace(/^https?:\/\//, '').replace(/\/$/, '')
      try {
        const resp = await axios.post('/api/integram-auth/switch-db', {
          myToken,
          targetDb: database,
          apiBase: serverHost,
        ...
        if (data.success && data.token) {
          // If proxy returned the system user (named after the DB), show 'my' user instead
          const isSystemUser = data.user === database
          const displayUser = isSystemUser ? (mySession?.userName || data.user) : data.user

          this.databases[database] = {
            token: data.token,
            xsrfToken: data.xsrf || mySession?.xsrfToken,
            userId: data.id || mySession?.userId,
          ...
          this.database = database
          this.token = data.token
          this.xsrfToken = data.xsrf || mySession?.xsrfToken
          this.userId = data.id || mySession?.userId
          this.userName = displayUser
          this.userRole = data.role || 'user'
          // If returned token == myToken, auth is still via 'my'; else via db directly
          this.authDatabase = data.token === myToken ? 'my' : database
          this.saveSession()
          console.log(`[integramApiClient] switchDatabase('${database}'): success via switch-db, authDatabase=${this.authDatabase}`)
          return true
        }
      } catch (err) {
        console.warn(`[integramApiClient] switchDatabase('${database}'): switch-db backend failed (${err.message}), falling back to my_token`)
      }

      // Fallback: register sub-db with my_token (PHP newDb() may copy it)
      this.databases[database] = {
        token: myToken,
        xsrfToken: mySession?.xsrfToken || this.xsrfToken,
        userId: mySession?.userId || this.userId,
        userName: mySession?.userName || this.userName,
        userRole: mySession?.userRole || this.userRole,
        ownedDatabases: [],
      }
      this.currentDatabase = database
      this.database = database
      ...
      this.authDatabase = 'my'
      this.saveSession()
      console.log(`[integramApiClient] switchDatabase('${database}'): fallback to my_token`)
      return true
    } catch (error) {
      console.error('[integramApiClient] Failed to switch database:', error)
      throw error
  ...
  getAuthHeaders(targetDatabase = null) {
    ...
    // Issue #6595: Correct header selection per Integram protocol
    //
    // Priority:
    // 1. databases[db].token (db-specific, obtained via switch-db exchange)
    //    — used when it DIFFERS from databases['my'].token, meaning it was obtained
    //      via cookie exchange because my_token doesn't work for this sub-database.
    // 2. databases['my'].token — fresh master token (kept up-to-date by validateSession)
    // 3. databases[db].token  — direct session token (fallback if no 'my' session)
    // 4. this.token           — legacy fallback
    const dbSpecificToken = this.databases[database]?.token
    const myToken = this.databases['my']?.token

    if (dbSpecificToken && myToken && dbSpecificToken !== myToken) {
      // db-specific token obtained via switch-db exchange — my_token didn't work
      headers['X-Authorization'] = dbSpecificToken
    } else if (myToken) {
      // Master 'my' token — works for all owned databases via PHP newDb() copy mechanism.
      headers['X-Authorization'] = myToken
    } else if (dbSpecificToken) {
      // Direct session token for this database (fallback if no 'my' session).
      headers['X-Authorization'] = dbSpecificToken
    } else if (this.token) {
      // Legacy fallback
      headers['X-Authorization'] = this.token
    } else {
      // Last resort: read my_token directly from localStorage.
      // Guards against databases['my'] not being populated yet (race condition on first load).
      const myTokenFromStorage = localStorage.getItem('my_token')
      if (myTokenFromStorage) {
        headers['X-Authorization'] = myTokenFromStorage
        // Eagerly populate databases['my'] so future calls skip this fallback
        if (!this.databases['my']) {
          this.databases['my'] = {
            token: myTokenFromStorage,
            xsrfToken: localStorage.getItem('my_xsrf') || myTokenFromStorage,
            userId: localStorage.getItem('my_id'),
            userName: localStorage.getItem('my_user'),
            userRole: 'user',
            ownedDatabases: []
          }
        } else {
          this.databases['my'].token = myTokenFromStorage
        }
      }
    }
  ...
  logout() {
    ...
    // Clear accumulated multi-database sessions to prevent memory leak
    this.databases = {}
  ...
  /**
   * Ensure we have an XSRF token for the current database.
   * Issue #6583: Each database has its own XSRF token. POST requests to a database
   * fail with "Неверный или устаревший токен CSRF" if using another database's XSRF.
   */
  async ensureXsrf() {
    const db = this.database
    if (this.databases[db]?.xsrfToken) return // Already have it

    try {
      const data = await this.get('xsrf')
      if (data?._xsrf) {
        if (!this.databases[db]) {
          this.databases[db] = {}
        }
        this.databases[db].xsrfToken = data._xsrf
        // Also update legacy property if this is the auth database
        if (db === this.authDatabase || db === 'my') {
          this.xsrfToken = data._xsrf
        }
        console.log(`[integramApiClient] Fetched XSRF for "${db}"`)
      }
    } catch (e) {
      console.warn(`[integramApiClient] Failed to fetch XSRF for "${db}":`, e.message)
    }
  }
   ...
   * Issue #6474: Added retry logic for SESSION_RESTORED_RETRY
   *
   * RACE CONDITION FIX: Capture the URL at call time so concurrent code that
   * temporarily changes this.database (e.g. getOwnedDatabases) cannot cause the
   * retry to go to a different database than intended.
   *
   * @param {string} endpoint - API endpoint
   * @param {object} params - Query parameters
   * @param {number} _retryCount - Internal retry counter
   * @param {string|null} _capturedUrl - Pre-built URL for retries (prevents database switch race)
   */
  async get(endpoint, params = {}, _retryCount = 0, _capturedUrl = null) {
    // Capture URL at call time so retries use the SAME database endpoint even if
    // concurrent code (e.g. getOwnedDatabases) temporarily changes this.database.
    // For the first call: build URL now. For retries: reuse the URL passed in.
    let requestUrl = null

    try {
      if (!this.isAuthenticated() && endpoint !== 'xsrf') {
        // Issue #6474: Try to restore session before throwing error
        if (this.tryRestoreSession()) {
          console.log('[integramApiClient] Session restored before GET request')
        } else {
          throw new Error('Not authenticated. Call authenticate() first.')
        }
      }

      // Build URL once; for retries _capturedUrl is the URL from the original call.
      requestUrl = _capturedUrl || this.buildURL(endpoint)
      ...
      // Issue #6474: Retry once if session was restored
      // Pass the captured URL so the retry goes to the SAME database even if
      // this.database was changed by concurrent code between the original call and the retry.
      if (handledError.message === 'SESSION_RESTORED_RETRY' && handledError.canRetry && _retryCount < 1) {
        console.log('[integramApiClient] Retrying GET request after session restore')
        // Issue #6627: For sub-databases, the cached token may be stale (loaded from
        // localStorage). Force a fresh switchDatabase call to get a new token before retry.
        const currentDb = this.database
        if (currentDb && currentDb !== 'my') {
          delete this.databases[currentDb]
          try {
            await this.switchDatabase(currentDb)
          } catch (e) {
            console.warn('[integramApiClient] switchDatabase failed during 401 recovery:', e.message)
          }
        }
        // Use requestUrl if it was built before the error, otherwise build now
        const urlForRetry = requestUrl || this.buildURL(endpoint)
        return this.get(endpoint, params, _retryCount + 1, urlForRetry)
      }
   ...
   * Issue #5112: Uses getAuthHeaders() for multi-database authorization
   * Issue #6474: Added retry logic for SESSION_RESTORED_RETRY
   * Issue #6501: Support for 'my' xsrf token for DDL operations
   *
   * @param {string} endpoint - API endpoint
   * @param {object} data - POST data
   * @param {object} options - Axios options + useMyXsrf flag for DDL
   * @param {number} _retryCount - Internal retry counter
   */
  async post(endpoint, data = {}, options = {}, _retryCount = 0) {
    try {
      if (!this.isAuthenticated()) {
        // Issue #6474: Try to restore session before throwing error
        if (this.tryRestoreSession()) {
          console.log('[integramApiClient] Session restored before POST request')
        } else {
          throw new Error('Not authenticated. Call authenticate() first.')
        }
      }

      const { useMyXsrf, useMyDatabase, ...axiosOptions } = options

      // Build URL
      const url = this.buildURL(endpoint)

      // Issue #6595: Match XSRF token to the auth token being used.
      // - Direct session (X-Authorization: db_token) → use db-specific XSRF
      // - Cross-db via 'my' master token (X-Authorization: my_token) → use 'my' XSRF
      // newDb() copies the same xsrf from 'my' to all owned databases, so both work.
      // Mismatch causes "Неверный или устаревший токен CSRF" error.
      const usingMyToken = !this.databases[this.database]?.token && !!this.databases['my']?.token
      let xsrfToUse = usingMyToken
        ? (this.databases['my']?.xsrfToken || this.xsrfToken)
        : (this.databases[this.database]?.xsrfToken || this.xsrfToken)

      // Add XSRF token to data
      const postData = new URLSearchParams()
      postData.append('_xsrf', xsrfToUse)
      ...
      // Issue #5112: Get authorization headers based on multi-database logic
      // CRITICAL FIX: Pass target database to ensure correct headers
      // Issue #6474: When useMyDatabase, get headers for 'my' database
      const targetDb = useMyDatabase ? 'my' : this.database
      const authHeaders = this.getAuthHeaders(targetDb)
      ...
      // Issue #6474: Retry once if session was restored
      if (handledError.message === 'SESSION_RESTORED_RETRY' && handledError.canRetry && _retryCount < 1) {
        console.log('[integramApiClient] Retrying POST request after session restore')
        return this.post(endpoint, data, options, _retryCount + 1)
      }
  ...
  handleError(error) {
    ...
    if (error.response) {
      ...
      if (status === 401) {
        ...
        // Session can't be restored — redirect to login if user had a session
        console.warn('[integramApiClient] Session could not be restored')
        const hadSession = localStorage.getItem('integram_session') || localStorage.getItem('my_token')
        if (hadSession && !this._redirectingToLogin && typeof window !== 'undefined') {
  ...
  //
  // 3. REQUISITE TYPES:
  //    - 3 = SHORT, 8 = CHARS, 2 = LONG, 13 = NUMBER, 4 = DATETIME, 7 = BUTTON
  ...
  /**
   * Save requisite attributes (default value, AI config, etc.)
   * DDL operation - requires 'my' session for non-'my' databases
   * Issue #6501: Uses 'my' xsrf token for cross-database DDL
   */
   ...
   *
   * @param {number} baseTypeId - Standard base type ID (e.g. 7 for BUTTON)
   * @returns {Promise<number>} Concrete type ID to use in _d_req
   */
  async resolveRequisiteType(baseTypeId) {
    try {
      const data = await this.getTypeEditorData()
      const et = data.edit_types || data
      const ids = et['0'] || et['id'] || []
      const baseTypes = et['1'] || et['t'] || []
      const names = et['4'] || et['val'] || []
      const ords = et['5'] || et['ord'] || []

      // Find type definitions (no ord) with matching baseType
      const candidates = []
      for (let i = 0; i < ids.length; i++) {
        const bt = parseInt(baseTypes[i])
        const ordVal = ords[i]
        const isTypeDef = !ordVal || ordVal === '' || ordVal === null
        if (isTypeDef && bt === baseTypeId) {
          candidates.push({ id: parseInt(ids[i]), name: names[i] || '' })
        }
      }
      ...
      // Prefer "Кнопка ИИ" for baseType=7, otherwise take the first match
      if (baseTypeId === 7) {
        const aiBtn = candidates.find(c => c.name.includes('ИИ') || c.name.includes('AI'))
        if (aiBtn) return aiBtn.id
      }

      return candidates[0].id
    } catch (error) {
      console.warn('[integramApiClient] resolveRequisiteType failed, using base type:', error.message)
      return baseTypeId
    }
  }

  /**
   * Resolve ALL concrete type IDs for a base type.
   * Issue #6662: When the first candidate returns a duplicate column,
   * other candidates can be tried as fallbacks.
   *
   * @param {number} baseTypeId - Standard base type ID (e.g. 7 for BUTTON)
   * @returns {Promise<number[]>} All concrete type IDs ordered by preference
   */
  async resolveAllRequisiteTypes(baseTypeId) {
    try {
      const data = await this.getTypeEditorData()
      const et = data.edit_types || data
      const ids = et['0'] || et['id'] || []
      ...
      // Sort: preferred type first (AI button for baseType=7)
      if (baseTypeId === 7) {
        candidates.sort((a, b) => {
          const aIsAI = a.name.includes('ИИ') || a.name.includes('AI') ? 0 : 1
          const bIsAI = b.name.includes('ИИ') || b.name.includes('AI') ? 0 : 1
          return aIsAI - bIsAI
        })
      }
      ...
      console.warn('[integramApiClient] resolveAllRequisiteTypes failed, using base type:', error.message)
      return [baseTypeId]
    }
  }

  /**
   * Create a new AI button type with an incremented numeric index.
   * Issue #6662: User requested "Кнопка ИИ 1", "Кнопка ИИ 2", etc. instead of
   * reusing existing concrete types.
   *
   * When all existing concrete button types return duplicate column IDs, this method
   * creates a new type definition (e.g. "Кнопка ИИ 2") by calling createType with
   * baseTypeId=7. The new type gets a unique ID that can be used with addRequisite()
   * to create a genuinely new column.
   *
   * @returns {Promise<number>} New type ID
   */
  async createAIButtonType() {
    // Find existing "Кнопка ИИ N" types to determine the next index
    let nextIndex = 2
    try {
      const data = await this.getTypeEditorData()
      const et = data.edit_types || data
      ...
      // Find all AI button type definitions (baseType=7, no ord)
      let maxIndex = 1
      for (let i = 0; i < ids.length; i++) {
        const bt = parseInt(baseTypes[i])
        const ordVal = ords[i]
        const isTypeDef = !ordVal || ordVal === '' || ordVal === null
        if (!isTypeDef || bt !== 7) continue

        const name = names[i] || ''
        // Match "Кнопка ИИ" (index 1, original) and "Кнопка ИИ N" (N >= 2)
        const baseMatch = name.match(/^Кнопка ИИ\s*(\d+)?$/)
        if (baseMatch) {
          const idx = baseMatch[1] ? parseInt(baseMatch[1]) : 1
      ...
      console.warn('[integramApiClient] createAIButtonType: could not determine index, using 2:', error.message)
      nextIndex = 2
    }

    const typeName = `Кнопка ИИ ${nextIndex}`
    console.log(`[integramApiClient] createAIButtonType: creating new type "${typeName}"`)
    const result = await this.createType(typeName, 7)
    const newTypeId = result.obj || result.id
    if (!newTypeId) {
      throw new Error(`createAIButtonType: failed to create type "${typeName}": ${JSON.stringify(result)}`)
    }
    console.log(`[integramApiClient] createAIButtonType: created "${typeName}" with ID ${newTypeId}`)
    return newTypeId
  }
  ...
  async uploadFile(file, path = '') {
    ...
    const headers = {
      ...
      ...this.getAuthHeaders(this.database)
    }
  ...
  // ============================================
  // Compatibility Aliases and Missing Methods
  // ============================================
  // These methods provide backward compatibility with integramService.js
  // After consolidation, code using integramService should work unchanged.

  /**
   * Authenticate with Integram API (integramService.js compatible signature)
   * @param {string} login - Username
   * @param {string} password - Password
   * @param {string} database - Database name (optional, uses current if not specified)
   * @returns {Promise<Object>} Authentication result with token and xsrf
   */
  async auth(login, password, database = null) {
    const db = database || this.database || this.currentDatabase || 'my'
    const result = await this.authenticate(db, login, password)
    // Convert to integramService format
    return {
  ...
  /**
   * Get session information (requires active auth)
   */
  async getSession() {
    if (!this.token) {
      throw new Error('No active session. Please login first.')
    }
    return await this.fetchSessionInfo()
  }
   ...
   * @param {Object} sessionData - Session data with token, xsrfToken, database, etc.
   */
  setSession(sessionData) {
    if (sessionData.token) this.token = sessionData.token
    if (sessionData.xsrfToken) this.xsrfToken = sessionData.xsrfToken
    if (sessionData.database) {
      this.database = sessionData.database
      this.currentDatabase = sessionData.database
    }
  ...
  /**
   * Get type metadata (alias for getTypeMetadata)
   */
  async getMetadata(typeId) {
    return this.getTypeMetadata(typeId)
  }

  /**
   * Get objects list (alias for getObjectList)
   */
  async getObjects(typeId, params = {}) {
    return this.getObjectList(typeId, params)
  }

  /**
   * Get object for editing (alias for getObjectEditData)
   */
  async getEditObject(objectId) {
    return this.getObjectEditData(objectId)
  }

  /**
   * Get types for editor (alias for getTypeEditorData)
   */
  async getEditTypes() {
    return this.getTypeEditorData()
  }
  ...
  async getObject(objectId) {
    return this.get(`object/${objectId}`, { full: 1 })
  }

  /**
   * Set object requisites (alias with different parameter format)
   * Uses _m_set endpoint which reliably saves all field types.
   */
  async setRequisites(objectId, requisites = {}) {
    return this.setObjectRequisites(objectId, requisites)
  }

  /**
   * Save requisite alias (alias for saveRequisiteAlias)
   */
  async saveAlias(requisiteId, alias) {
    return this.saveRequisiteAlias(requisiteId, alias)
  }

  /**
   * Save default value (alias for saveRequisiteAttributes)
   */
  async saveDefaultValue(requisiteId, attributes) {
    return this.saveRequisiteAttributes(requisiteId, attributes)
  }

  /**
   * Toggle required flag (alias for toggleRequisiteNull)
   */
  async toggleRequired(requisiteId) {
    return this.toggleRequisiteNull(requisiteId)
  }

  /**
   * Toggle multiselect flag (alias for toggleRequisiteMulti)
   */
  async toggleMultiselect(requisiteId) {
    return this.toggleRequisiteMulti(requisiteId)
  }

  /**
   * Create reference (alias for createTypeReference)
   */
  async createReference(requisiteId) {
    return this.createTypeReference(requisiteId)
  }

  /**
   * Move object to new parent (alias for moveObjectToParent)
   */
  async moveObject(objectId, newParentId) {
    return this.moveObjectToParent(objectId, newParentId)
  }

  /**
   * Update object (compatible with integramService format)
   * @param {number} objectId - Object ID
   * @param {Object} data - Object data with typeId, value, and optionally reqs
   */
  async updateObject(objectId, data) {
    if (data.typeId && data.value !== undefined) {
      const requisites = {}
      if (data.reqs) {
        Object.assign(requisites, data.reqs)
      } else {
        // Extract requisites from top-level keys
        for (const [key, value] of Object.entries(data)) {
          if (key !== 'value' && key !== 'typeId' && key !== 'id' && key !== 'reqs') {
            requisites[key] = value
          }
        }
      }
      return this.saveObject(objectId, data.typeId, data.value, requisites)
    }
    throw new Error('Invalid object data format')
  }

  /**
   * Get list of all reports (objects of type 22)
   * @param {Object} params - Filter parameters
   * @returns {Promise<Object>} Report list with metadata
   */
  async getReportList(params = {}) {
    const REPORT_TYPE_ID = 22
    return this.getObjectList(REPORT_TYPE_ID, params)
  }

  /**
   * Get report details for editing
   * @param {number} reportId - Report object ID
   * @returns {Promise<Object>} Report data including SQL query and settings
   */
  async getReport(reportId) {
    return this.getObjectEditData(reportId)
  }
   ...
   * @param {number} reportId - Report object ID
   * @param {string} name - Report name
   * @param {Object} data - Report data (SQL query, settings, etc.)
   * @returns {Promise<Object>} Updated report object
   */
  async updateReport(reportId, name, data = {}) {
    const REPORT_TYPE_ID = 22
    const requisites = {}

    if (data.sqlQuery) {
      requisites['sql'] = data.sqlQuery
    }
    if (data.settings) {
      Object.assign(requisites, data.settings)
    }

    return this.saveObject(reportId, REPORT_TYPE_ID, name, requisites)
  }

  /**
   * Delete report (alias for deleteObject)
   * @param {number} reportId - Report object ID
   * @returns {Promise<Object>} Deletion result
   */
  async deleteReport(reportId) {
    return this.deleteObject(reportId)
  }

  /**
   * Build filter parameters for object list
   * Supports: FR_{field}, TO_{field}, F_{field}, search
   */
  buildFilters(filters) {
    ...
    // Date range filters
    if (filters.dateFrom) {
      for (const [field, value] of Object.entries(filters.dateFrom)) {
        params[`FR_${field}`] = value
      }
    }

    if (filters.dateTo) {
      for (const [field, value] of Object.entries(filters.dateTo)) {
        params[`TO_${field}`] = value
      }
    }
  ...
  /**
   * Parse object data from API response for editing
   */
  parseObjectForEdit(apiResponse) {
    const { obj, reqs } = apiResponse
    ...
    if (reqs) {
      for (const [reqId, reqData] of Object.entries(reqs)) {
        formData.requisites[reqId] = reqData.value || ''
      }
    }
  ...
  /**
   * Parse object list data from API response
   */
  parseObjectList(apiResponse) {
    const { type, object, reqs, req_order, req_type } = apiResponse

    if (!object) return { objects: [], columns: [] }
    ...
    columns.push({
      field: 'val',
      header: type?.val || 'Value',
      sortable: true
    })

    if (req_order) {
      for (const reqId of req_order) {
        columns.push({
          field: `req_${reqId}`,
          header: req_type?.[reqId] || `Field ${reqId}`,
          sortable: true
        })
      }
    }
  ...
  /**
   * Get reference requisites for a type
   * @param {number} requisiteId - Requisite ID
   * @param {Object} options - Options
   * @returns {Promise<Object>} Reference requisites data
   */
  async getRefReqs(requisiteId, options = {}) {
    const params = { ...options }
    return this.get(`ref_reqs/${requisiteId}`, params)
  }

  /**
   * Execute connector/action on object
   * @param {number} objectId - Object ID
   * @param {Object} params - Connector parameters
   * @returns {Promise<Object>} Connector result
   */
  async executeConnector(objectId, params = {}) {
    return this.post(`connector/${objectId}`, params)
  }
   ...
   */
  async massDeleteObjects(objectIds) {
    const results = []
    for (const objectId of objectIds) {
      try {
        const result = await this.deleteObject(objectId)
        results.push({ objectId, success: true, result })
      } catch (error) {
        results.push({ objectId, success: false, error: error.message })
      }
    }
    return results
  }
  ...
  /**
   * Modify requisite attributes
   * @param {number} requisiteId - Requisite ID
   * @param {string} attributes - Attributes string
   * @returns {Promise<Object>} Result
   */
  async modifyRequisiteAttributes(requisiteId, attributes) {
    return this.saveRequisiteAttributes(requisiteId, attributes)
  }

  /**
   * Change requisite type
   * @param {number} requisiteId - Requisite ID
   * @param {number} newTypeId - New type ID
   * @returns {Promise<Object>} Result
   */
  async changeRequisiteType(requisiteId, newTypeId) {
    return this.post(`_d_type/${requisiteId}`, { t: newTypeId })
  }

  /**
   * Dir admin (alias for getDirAdmin)
   * @param {string} path - Path
   * @returns {Promise<Object>} Directory data
   */
  async dirAdmin(path = '') {
    return this.getDirAdmin(path)
  }
}
