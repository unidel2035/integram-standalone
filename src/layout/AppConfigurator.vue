...
const savedConfig = JSON.parse(localStorage.getItem('appThemeConfig')) || {
  preset: layoutConfig.preset,
  menuMode: layoutConfig.menuMode,
  primary: layoutConfig.primary || 'blue',
  surface: layoutConfig.surface || (isDarkTheme.value ? 'zinc' : 'slate')
};

const appConfig = ref(savedConfig);

watch(appConfig, (newVal) => {
  localStorage.setItem('appThemeConfig', JSON.stringify(newVal));
  applyTheme();
}, { deep: true });
...
const preset = ref(appConfig.value.preset);
const presetOptions = ref(Object.keys(presets));

const menuMode = ref(appConfig.value.menuMode);
...
function getPresetExt() {
  const color = primaryColors.value.find((c) => c.name === appConfig.value.primary);
...
function applyTheme() {
  layoutConfig.preset = appConfig.value.preset;
  layoutConfig.menuMode = appConfig.value.menuMode;
  layoutConfig.primary = appConfig.value.primary;
  layoutConfig.surface = appConfig.value.surface;

  const presetValue = presets[appConfig.value.preset];
  const surfacePalette = surfaces.value.find((s) => s.name === appConfig.value.surface)?.palette;
...
function updateColors(type, color) {
  if (type === 'primary') {
    appConfig.value.primary = color.name;
  } else if (type === 'surface') {
    appConfig.value.surface = color.name;
  }
}

function onPresetChange() {
  appConfig.value.preset = preset.value;
}

function onMenuModeChange() {
  appConfig.value.menuMode = menuMode.value;
}

const fileInput = ref(null);

const handlePresetLogoChange = (preset) => {
  setPresetLogo(preset.name)
  getToast().add({
    severity: 'success',
    summary: 'Логотип изменён',
    detail: `Выбран логотип: ${preset.label}`,
    life: 2000
  })
}

const handleDynamicColorChange = (color) => {
  setDynamicColor(color.name)
  getToast().add({
    severity: 'success',
    summary: 'Цвет логотипа изменён',
    detail: `Цвет: ${color.label}`,
    life: 2000
  })
}
...
const handleFileUpload = (event) => {
  ...
  if (!file.type.includes('svg')) {
    getToast().add({
      severity: 'error',
      summary: 'Ошибка',
      detail: 'Пожалуйста, загрузите SVG файл',
      life: 3000
    });
    return;
  }
  ...
  reader.onload = (e) => {
    ...
    setCustomLogo(svgContent);
    getToast().add({
      severity: 'success',
      summary: 'Логотип загружен',
      detail: 'Пользовательский логотип успешно установлен',
      life: 3000
    });
    // Reset file input
    if (fileInput.value) {
...
const handleResetLogo = () => {
  ...
  getToast().add({
    severity: 'info',
    summary: 'Логотип сброшен',
    detail: 'Установлен логотип по умолчанию',
    life: 2000
  });
};
  ...
  <div class="config-panel">
    <div class="config-panel-content">
      <div class="config-panel-colors">
        ...
        <div>
          <button
            ...
            :class="{ 'active-color': appConfig.primary === primaryColor.name }"
      ...
      <div class="config-panel-colors">
        ...
        <div>
          <button
            ...
            :class="{ 'active-color': appConfig.surface === surface.name }"
      ...
      <div class="config-panel-settings">
        ...
        <SelectButton v-model="preset" @change="onPresetChange" :options="presetOptions" :allowEmpty="false" size="small" />
      </div>
      <div class="config-panel-settings">
        <span class="config-panel-label">Menu Mode</span>
        <SelectButton v-model="menuMode" @change="onMenuModeChange" :options="menuModeOptions" :allowEmpty="false" optionLabel="label" optionValue="value" size="small" />
      </div>

      <div class="config-panel-colors">
        <span class="config-panel-label">Готовые логотипы</span>
        <div class="logo-preset-grid">
          <button
            v-for="preset of presetLogos"
            :key="preset.name"
            type="button"
            :title="preset.label"
            @click="handlePresetLogoChange(preset)"
            :class="{ 'active-logo': logoConfig.type === 'preset' && logoConfig.preset === preset.name }"
            class="logo-preset-button"
          >
      ...
      <div class="config-panel-colors">
        ...
        <div class="logo-color-grid">
          <button
            ...
            type="button"
            :title="color.label"
            @click="handleDynamicColorChange(color)"
            :class="{ 'active-color': logoConfig.type === 'dynamic' && logoConfig.color === color.name }"
            :style="{ backgroundColor: color.color }"
            class="logo-color-button"
          ></button>
        </div>
      </div>

      <div class="config-panel-settings logo-upload-section">
...
.logo-preset-button img {
  max-width: 100%;
  max-height: 100%;
  object-fit: contain;
}

.logo-preset-button:hover {
  transform: scale(1.05);
  border-color: var(--primary-color);
}

.logo-preset-button.active-logo {
  border-color: var(--primary-color);
  border-width: 3px;
...
.logo-color-button:hover {
  transform: scale(1.1);
  border-color: var(--surface-border);
}

.logo-color-button.active-color {
  border-color: var(--primary-color);
