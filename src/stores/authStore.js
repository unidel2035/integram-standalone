...
export const useAuthStore = defineStore('auth', () => {
   ...
   * Issue #6377: Use backend proxy to avoid CORS issues with Integram
   */
  async function authenticateToDatabase(login, password, apiBase, database) {
    // Issue #6377: Use backend proxy endpoint instead of direct Integram requests
    // This avoids CORS errors when accessing through proxy.drondoc.ru
    try {
      const response = await axios.post('/api/integram-auth/authenticate', {
        login,
        password,
        apiBase,
        database
      ...
      // If proxy endpoint fails (e.g., backend down), fall back to direct request
      // This ensures backward compatibility
      console.warn('[authStore] Proxy endpoint failed, trying direct request:', error.message)
