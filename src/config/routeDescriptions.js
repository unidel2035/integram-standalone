...
export const routeDescriptions = {
  ...
  '/fst-soft-model': {
    ...
    category: 'tools',
    tags: ['Software Model', 'Event Ontology', 'Meta', 'DevOps', 'Health Score', 'Gap Analysis'],
  },
  ...
  '/event-ontology': {
    description: 'Движок событийной онтологии ФСТ НТИ: 24 панели — дашборд, акторы, концепты, словари, модели, граф событий, конструктор запросов, сценарии, верификация, требования, SysML, цифровой двойник, AI инвесткомитет, суверенность, канбан, таймлайн. Визуальный редактор моделей, конечные автоматы, потоковая обработка событий.',
    category: 'ai',
    tags: ['ФСТ', 'НТИ', 'Онтология', 'События', 'Модели', 'SysML', 'Цифровой двойник', 'Суверенность', 'Канбан']
  },
  '/fst-committee': {
  ...
  '/fst-flow': {
    description: 'ФСТ НТИ — Моя панель: единая адаптивная страница для пользователей. Контент пересобирается из событийных компонентов на основе роли (стартап, инвестор, эксперт, аналитик, директор) и текущего состояния онтологии. Стартап видит прогресс заявки и следующий шаг. Инвестор — метрики портфеля и сигналы. Директор/аналитик — Command Center mini + конвейер сделок. Эксперт — предстоящие сессии ИК и задачи. AI-брифинг на день (DeepSeek).',
    category: 'fund',
    tags: ['ФСТ', 'НТИ', 'Роль', 'Адаптивный', 'Панель', 'Стартап', 'Инвестор', 'Эксперт', 'Аналитик', 'Онтология', 'AI', 'Events']
  },
  '/fst-pipeline': {
    description: 'ФСТ НТИ — Регуляторный конвейер ПП-1→ПП-6: живая визуализация полного инвестиционного процесса по Регламенту ФСТ НТИ (утв. 23.03.2023). Шесть подпроцессов: ПП-1 Регистрация ПИ, ПП-2 Предварительный анализ, ПП-3 ИК-1, ПП-4 Экспертизы + ИК-2, ПП-5 Закрытие сделки, ПП-6 Мониторинг и выход. 37 регламентных событий из онтологии Integram (концепты #70910-71157). AI-анализ блокировщиков (DeepSeek), прогноз следующих шагов (Claude), compliance-отчёт по регламенту. Кросс-сущностные связи: session→deal→company→fund→exit. SLA-контроль рабочих дней по каждому подпроцессу.',
    category: 'fund',
    tags: ['ФСТ', 'НТИ', 'Регламент', 'Конвейер', 'ПП-1', 'ПП-2', 'ПП-3', 'ПП-4', 'ПП-5', 'ПП-6', 'Онтология', 'AI', 'Инвесткомитет', 'SLA', 'Events', 'Pipeline', 'Compliance']
  },
  '/fst-present': {
    description: 'ФСТ НТИ — Режим презентации: телесуфлёр для Гордина + двухоконная синхронизация через BroadcastChannel. Консоль режиссёра показывает сценарий и подсказки, чистое окно для Пескова автоматически переключает страницы.',
    category: 'fund',
    tags: ['Презентация', 'Телесуфлёр', 'Демо', 'Гордин', 'Песков']
  },
  '/fst-billing': {
    description: 'Биллинг и подписка — управление тарифным планом (Бесплатный / Профессиональный / Корпоративный), мониторинг расхода AI-токенов по моделям и модулям, история вызовов, квота использования.',
    category: 'fund',
    tags: ['Биллинг', 'Подписка', 'Тариф', 'AI Usage', 'Токены', 'Оплата']
  },
  '/fst-learning': {
  ...
  '/fst-apply': {
    description: 'ФСТ НТИ — Подача заявки стартапом: многошаговая форма для подачи заявки в фонд (Issue #84). 4 шага: Компания (название, ИНН, сфера, стадия), Технология (TRL, патенты, рынок TAM/SAM, суверенность), Финансы (запрашиваемая сумма, pre-money, команда, достижения), Документы (pitch deck, финмодель, контакты). Прогресс-бар, inline-подсказки по TRL/MRL/суверенности, предварительный скоринг с gate-проверкой, сохранение в api.ai2o.ru/fst через Integram. После отправки: статус заявки и next steps (скрининг → DD → ИК → term sheet). Канонический код: github.com/unidel2035/found',
    category: 'finance',
    tags: ['ФСТ', 'НТИ', 'Заявка', 'Стартапы', 'Форма', 'TRL', 'MRL', 'Суверенность', 'Скоринг', 'Integram', 'Венчур', 'БПЛА', 'found', 'Issue #84']
  },
  '/fst-gov': {
    description: 'GR-панель Government Relations: персональный план мер господдержки для портфельных стартапов, событийная онтология GR-ленты (Event Sourcing), маршрут финансирования TRL→9, контрфактический анализ "что если бы события не было", пробелы в поддержке по онтологическому выводу, конструктор новых мер через AI из регуляторных изменений, регуляторный радар (карта НПА по уровню риска). Меры: УМНИК, СТАРТ-1/2, Сколково, Нацпроект БАС, РФРИТ, Минпромторг, ВЭБ.РФ, ФПИ, НТИ Аэронет.',
    category: 'business',
    tags: ['GR', 'господдержка', 'меры поддержки', 'онтология', 'событийная лента', 'TRL', 'контрфактический анализ', 'Нацпроект БАС', 'Сколково', 'УМНИК', 'СТАРТ', 'ФРП', 'НТИ', 'регуляторный радар'],
  },
  '/fst-room': {
    description: 'Agent Room — живой чат для агентов и людей через SSE (Server-Sent Events). Агенты могут подключаться к именованным комнатам через POST /api/room/send, получать сообщения через SSE /api/room/stream. Используется для координации агентов в реальном времени, мультиагентных сессий.',
    category: 'ai',
    tags: ['Agent Room', 'SSE', 'живой чат', 'агенты', 'мультиагентность', 'координация', 'real-time'],
  },
  '/fst-startuper': {
    description: 'AI-агент первого касания — принимает документы стартапа, задаёт вопросы, строит цифровой двойник, выявляет аномалии и готовит пакет маяков для инвесткомитета',
    category: 'business',
    tags: ['стартапер', 'заявка', 'первое касание', 'цифровой двойник', 'парсинг', 'психотип', 'маяки'],
  },
  '/fst-event-graph': {
    description: 'ФСТ НТИ — Событийный граф: матрица переходов между типами событий портфельных компаний (Финансовое/Технологическое/Рыночное/Регуляторное), бэктест предсказательной силы онтологии, точность по компаниям, визуализация confusion matrix.',
    category: 'ai',
    tags: ['события', 'граф', 'матрица переходов', 'бэктест', 'онтология', 'предсказательная сила', 'портфель', 'ФСТ'],
  },
  '/fst-factor': {
    description: 'ФСТ НТИ — Факторная модель T·S·M·G·E (Aladdin для ФСТ): оценка венчурной сделки по 5 факторам — TRL Momentum, Sovereign Score, Multiplier Score, GR Coverage, Export Readiness. Веса, пороги и стресс-сценарии хранятся в Integram DB. Детализация факторов, AI-обоснование, стресс-тест из 5 сценариев, влияние на портфель.',
    category: 'ai',
    tags: ['факторная модель', 'Aladdin', 'TRL', 'суверенность', 'мультипликатор', 'стресс-тест', 'портфель', 'ФСТ', 'оценка'],
  },
  '/fst-project': {
    description: 'ФСТ НТИ — Хаб проекта: сквозной маршрут компании через инвестиционный цикл. Pipeline стадий (заявка → due dil → факторы → ИК → сделка → портфель), лента всех событий (FACTOR_SCORED, VOTE_CAST, DECISION_MADE, TERM_SHEET_PROPOSED, DEAL_CLOSED), переходы на любую страницу с контекстом проекта.',
    category: 'fund',
    tags: ['проект', 'pipeline', 'инвестиционный цикл', 'события', 'ФСТ', 'хаб'],
  },
  '/fst-sourcing': {
  ...
  '/tech-pyramid': {
    description: 'Пирамида технологий — интерактивная SVG-визуализация технологического суверенитета России из БД api.ai2o.ru/tech. 9 категорий технологий, 45 технологий из TSM PDF стр.15, клик по точке открывает модальное окно с описанием.',
    category: 'visualization',
  ...
  '/auth.asp': {
    description: 'OAuth callback для Integram: обработка авторизации через протокол OAuth при входе в систему Integram (api.ai2o.ru)',
    category: 'auth',
  ...
  '/integram-browser': {
    description: 'Браузер баз данных Integram: просмотр и управление таблицами, объектами и реквизитами в базах данных api.ai2o.ru (my, kval, tech и др.)',
    category: 'data',
    tags: ['Integram', 'Browser', 'Database', 'Tables', 'api.ai2o.ru']
  },
  ...
  '/my': {
    description: 'Вход в Integram: авторизация в системе Integram через логин/пароль, выбор сервера (api.ai2o.ru), управление сессией',
    category: 'auth',
