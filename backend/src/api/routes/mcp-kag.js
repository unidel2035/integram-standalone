...
async function handleCreateEntities(kagService, args) {
  ...
  for (const entityData of entities) {
    const id = `${entityData.entityType || 'entity'}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const entity = {
      id,
      name: entityData.name,
      type: entityData.entityType || 'Entity',
      observations: Array.isArray(entityData.observations) ? entityData.observations : [],
      properties: entityData.properties || {},
      source: 'manual',
      updated_at: new Date().toISOString()
    };
    kagService.addEntity(entity);
    created.push(entity);
    // Fire-and-forget embedding generation
    kagService.generateEntityEmbedding(entity).catch(() => {});
  }
